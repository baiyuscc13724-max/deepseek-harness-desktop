...
- 修复 Computer Use 插件卡与桌面宿主之间缺失的状态/动作桥接；授权后改为捕获整个 Windows 虚拟桌面并使用全局坐标输入，不再选择窗口或设置内容级敏感操作过滤；永久授权会在应用启动时自动恢复，并在锁屏或挂起结束后恢复此前开启的会话。
- Computer Use 控制指示现在以不拦截鼠标、且不进入截图的透明置顶层覆盖每一块显示器，而不再只覆盖 Harness Desktop 自身窗口。
