...
- 合并桌面安装包、签名组件与 PR 预览候选为单一更新中心，保留签名校验、显式应用、健康检查与稳定版本回滚边界。
- 修复 Computer Use 插件卡与桌面宿主之间缺失的状态/动作桥接；永久授权现在会在应用启动时自动恢复共享控制，并在锁屏或挂起结束后恢复此前开启的永久会话。
- 为未被上游正确分类的 Codex 服务过载错误加入可取消、可观察的五次有界退避重试；重试耗尽后仍保留会话上下文供稍后继续。
