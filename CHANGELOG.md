...
- 桌面更新清单及发布资产支持多地址优先级与自动换源，CNB 国内镜像可以排在 GitHub 前面。
- 设置页明确区分可安装的桌面更新与等待兼容发布的官方核心更新，避免出现“发现新版但没有更新按钮”的误导状态。
- 增加 Provider 用量适配、安装目录内独立运行时工作区和国内镜像清单生成工具；所有安装来源继续强制校验大小与 SHA-256。
...
- 清理缓存只识别系统“清除缓存”入口，无法安全识别时停留在应用设置页并提示手动完成，绝不点击“清除数据”。
- 保留原扫码配对、会话同步、实时刷新、官方设置结构和安装数据；覆盖升级不会清除现有配对。
