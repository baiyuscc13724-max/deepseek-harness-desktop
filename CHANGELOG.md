...
- 以缺陷修复和运行减负为主：修复启动注入、Windows 覆盖安装文件占用、右侧浏览器反复开关、Git 浏览器授权重复弹码与不刷新，以及旧版客户端更新重定向/校验兼容问题；完整性签名与 SHA-256 门禁不降级。
...
- 桌面、全部随包内置插件、Android 与 iOS/iPadOS 源码版本同步到 1.0.37；Android `versionCode` 更新为 10037。
