...
- 合入 PR #47：Desktop 仅在浏览器安全的动态端口范围内启动独立回环运行时；团队 SSE 首次握手有界超时并释放停滞连接，不削弱 Chromium 安全策略。
- 修复手机 WSS 重连中旧连接事件清理新代连接的竞态；工作区清单 HTTP/WS 鉴权与取消传播一致，连接 teardown 吸收关闭阶段错误并有界收敛。
- 手机连接提示不再把页面存在当作已连接，列表恢复耗尽后给出明确重试；预览附件交互、焦点与可访问性同步 Android/iOS 资源。
- 手机控制严格绑定配对设备连接与回执身份，拒绝陈旧或不匹配结果；同步既有回归契约及版本身份，不新增权限。
- 官方 Harness 仍固定 `0.1.2-alpha.5`。本地仅整合已验证修复，未追加测试或现场复现；发布器安全门禁、云端构建签名与正式制品验收仍须完成，外网故障现场因果尚未确认。既有 v1.0.60 Tag、资产与 stable feed 保持不可变。

## 1.0.60

官方 Harness 依赖维持 `0.1.2-alpha.5`，不升级依赖或扩展功能范围。

- 团队范围提醒区分开工时与开工后新增的内部任务；提醒对照用户原始目标收口，不将任务数量冒充需求授权。
- `team_status.wait_for_change` 在当前轮次内被动等待真实事件，避免反复查询和无变化汇报；可随 Stop 取消，不操作 Goal、补发授权或唤醒成员。
- 精确根会话在直接用户回合中可安全取消暂停任务、关闭已停止团队，无须先 Resume；保留 OCC、pause epoch、身份和取消历史，排空失败仍保持暂停。
- 本版本仅含上述机制修复及必要版本/发布验证，不纳入已取消的浏览器、离屏或原生预览任务。
