...
- Agent Teams 现在会明确区分“调用者无权访问团队”和“目标成员已失败、不可接收任务”，避免任务分配失败被误报为根负责人会话失效；失败成员继续保持 fail-closed，不能被重新分配未完成任务。
- Codex 临时过载改为通过持久化可见重试事件执行最多五次、可取消的指数退避；仅窄匹配官方过载错误，认证、余额及其他提供商错误不会被重试，耗尽后会保留当前会话并给出恢复说明。
- 修复 Computer Use 截图坐标被模型按源分辨率重复缩放后触发“坐标超出可控区域”的问题：点击和滚动只接受最新截图 `width`/`height` 的像素坐标，由宿主完成实际目标映射；截图结果不再暴露易混淆的源分辨率，并返回明确的输入边界与越界诊断。
- 已安装的签名 PR 预览会被持久记住：同一份预览即使重新发布也不会再次提示安装，只有内容真正变化时才会显示为新的更新。
...
- 文件精确编辑找不到原锚点时，会停止原样重试并提示重新读取文件，只允许基于最新内容重建一次短而唯一的匹配；仍不会使用可能误改代码的模糊替换。
- 会话列表的置顶和未读状态改由桌面端持久保存；应用重启或本地运行端口变化后仍会恢复，并继续限制为当前本机 Harness 界面访问。
- 修复打包版模型选择器缺少服务商模型目录的问题；现在优先读取随应用附带的完整目录，并在不可用时安全回退到已安装目录。
- 右侧工作区现在可安全预览图片、音频、视频、PDF 和有界文本；用户明确点击的本机绝对路径可在工作区外只读预览，其他类型可交给系统应用打开；图片生成等工具返回的持久图片也会直接显示在对话中，不再只显示 JSON。
