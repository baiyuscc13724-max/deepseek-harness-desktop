...
8. **托管 Runner 预装较新 Inno 导致固定版本安装失败**：Chocolatey 6.7.0 包和内置上游安装器哈希有效，但 Runner 已有更新版本时默认拒绝降级；恢复流程对固定 6.7.0 显式 `--allow-downgrade --force`，仍运行语言哈希、安装/卸载和制品门禁，且不移动既有 Tag。
