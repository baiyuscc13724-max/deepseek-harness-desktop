...
- runs `33944441121` 与 `33946203388` 的 macOS/Linux ordinary phase 都在输出前 17 个有序文件后等待到云端 60 分钟上限，run `33949989349` 又证明仅让插件 Store settle 仍不足：同文件的测试 peer Store 尚未关闭时会继续成为共享 retention owner。Cordis 的真实子 plugin fiber 已逐个等待 `fiber.dispose()`，临时 tool、admission、订阅及 service effect 也按逆序显式清理；最终 teardown 先关闭 peer，再由插件 `closeAndSettle()` 关闭最后实例、使 token 失效并等待共享 file-scoped maintenance 完整 settle。单文件 runtime 37/37、hot/cold 74/74 与完整 ordinary 2357 pass/29 intentional skip 均自然退出；没有使用 `--test-force-exit`、缩短测试、吞掉活动 handle、改变并发或重分组。
