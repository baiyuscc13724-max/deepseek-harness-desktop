...
- 独立 Ubuntu 诊断 run `33953021690` 在 2m18s 内给出可定位证据：plugin-service 8/8、3.27s 自然退出；runtime 28 pass/8 fail/1 timeout，具体超时为 `explicit Stop removes queued project wakes and fences enqueue resolution before ack` 的 `await started`。夹具写死 `C:/wake-stop` 等 Windows 路径，Unix 的 `isAbsolute` 正确拒绝，因此 `followup` 从未执行却被测试无限等待。修复仅将 wake 夹具改为 `path.resolve(os.tmpdir(), ...)`，保持产品绝对路径和 canonical identity 检查；Stop 测试用 `Promise.race` 检测 dispatch 提前结束并立即失败，finally 释放 gate 并结算 dispatch。不得把诊断 timeout 记为通过。修复提交 `28d5e77fc57cd6754028ca65655926403a152b6d` 的独立 Ubuntu run `33953408211` 已在 58 秒完成：plugin-service 8/8（3.275s）、runtime 37/37（10.805s），均 0 fail/0 skip 自然退出，原阻塞 Stop 测试为 17.936ms；本地 Windows runtime 同为 37/37。该证据只证明定向回归，不替代正式全平台发布门禁。
