...
const PLUGIN_ID = 'desktop-public-web-search'
const PLUGIN_PACKAGE = 'dsh-desktop-web-search'
...
async function ensurePatchEntry(file) {
  ...
  if ((rows || []).some(row => row?.insert?.some?.(entry => entry?.id === PLUGIN_ID || entry?.name === PLUGIN_PACKAGE))) return false
  if (rows === null) {
...
async function ensureDesktopWebSearchPlugin({ dshHome, bundledRoot }) {
  ...
  if (manifest.name !== PLUGIN_PACKAGE || manifest.type !== 'module' || !manifest.main) throw new Error('内置独立联网搜索插件包无效。')
  ...
  return {
    destination,
    patchChanged: await ensurePatchEntry(path.join(profile, 'cordis.patch.yml')),
    version: manifest.version
  }
}

module.exports = { PLUGIN_ID, PLUGIN_PACKAGE, ensureDesktopWebSearchPlugin, ensurePatchEntry }
