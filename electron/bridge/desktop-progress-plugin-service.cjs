...
const PLUGIN_ID = 'desktop-progress'
const PLUGIN_PACKAGE = 'dsh-desktop-progress'
...
async function ensureDesktopProgressPlugin({ dshHome, bundledRoot }) {
  ...
  if (manifest.name !== PLUGIN_PACKAGE) throw new Error('内置进度汇报插件包无效。')
...
module.exports = { PLUGIN_ID, PLUGIN_PACKAGE, ensureDesktopProgressPlugin, ensurePatchEntry }
