...
const PLUGIN_ID = 'session-experience'
const PLUGIN_PACKAGE = 'dsh-session-experience'
...
async function ensureSessionExperiencePlugin({ dshHome, bundledRoot }) {
  ...
  if (manifest.name !== PLUGIN_PACKAGE || typeof manifest.version !== 'string') {
    throw new Error('内置会话与附件体验插件包无效。')
  }
  if (!manifest.dsh?.client || manifest.dsh.client.platform !== 'web') {
    throw new Error('内置会话与附件体验插件缺少 Web 客户端声明。')
  }
  ...
  try {
    ...
    if (rollbackErrors.length) throw new AggregateError([error, ...rollbackErrors], '会话与附件体验插件安装失败且回滚未能完整完成。')
    throw error
  } finally {
