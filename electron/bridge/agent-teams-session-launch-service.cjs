...
function resolvedWorkspacePath(value) { const raw = text(value, 'workspacePath', 4096); if (!path.isAbsolute(raw)) throw serviceError('workspacePath is invalid', 'HOST_SESSION_LAUNCH_INVALID', true); return path.resolve(raw) }
function normalizedScope(value) { const normalized = resolvedWorkspacePath(value).replace(/\\/gu, '/'); return process.platform === 'win32' ? normalized.toLocaleLowerCase('en-US') : normalized }
function projectKeyForWorkspace(workspacePath) { return fingerprint(['agent-teams-project-v1', normalizedScope(workspacePath)]) }
function canonicalPath(value) { return normalizedScope(value) }
...
function createAgentTeamsSessionLaunchService({ stateFile, callRuntimeRpc, inspectRuntimePrompt = async () => null, endpoint = defaultEndpoint(), token = randomBytes(32), maxMessageBytes = MAX_MESSAGE_BYTES, maxTerminalRecords = MAX_TERMINAL_RECORDS, maxConcurrent = 2, maxConcurrentPerProject = 1, maxSessionsPerBatch = 8, socketIdleTimeoutMs = 30_000 } = {}) {
  ...
  let document, server, callerSalt, closed = false, closePromise, writeChain = Promise.resolve(), running = 0, projectCursor = 0, pumpScheduled = false
  const runningByProject = new Map(), queues = new Map(), pendingByOperation = new Map(), projectOrder = [], activeJobs = new Set(), acceptedRequests = new Set(), acceptedSockets = new Set()
  const bindingsByCaller = new Map(), bindingsByTicket = new Map(), operationsByRef = new Map(), operationsBySlot = new Map(), reconciliationChains = new Map()
  ...
  const resolveBinding = async request => {
    ...
    const workspacePath = resolvedWorkspacePath(request.workspacePath), laneRef = laneRefFor(canonicalProjectKey)
  ...
  const settleClosedEnqueue = (binding, operation) => {
    if (operation.state !== 'queued') return Promise.resolve(publicOperation(binding, operation))
    operation.state = 'cancelled'; operation.phase = 'cancelled'; operation.updatedAt = Date.now()
    return persist(true).then(() => publicOperation(binding, operation))
  }
  const enqueue = (binding, operation) => { if (closed) return settleClosedEnqueue(binding, operation); const pendingKey = operationRefKey(binding.laneRef, operation.operationRef), old = pendingByOperation.get(pendingKey); if (old) return old; const pending = new Promise((resolve, reject) => { const queue = queues.get(binding.laneRef) || []; queue.push({ binding, operation, resolve, reject }); queues.set(binding.laneRef, queue); if (!projectOrder.includes(binding.laneRef)) projectOrder.push(binding.laneRef); schedulePump() }).finally(() => pendingByOperation.delete(pendingKey)); pendingByOperation.set(pendingKey, pending); return pending }
  function recoverableBinding(binding) {
    try { return typeof binding?.workspacePath === 'string' && typeof binding.canonicalProjectKey === 'string' && projectKeyForWorkspace(binding.workspacePath) === binding.canonicalProjectKey && laneRefFor(binding.canonicalProjectKey) === binding.laneRef }
    catch { return false }
  }
  function resumeQueuedForBinding(binding) {
    if (!recoverableBinding(binding)) return
    for (const operation of document.operations) {
      if (operation.state !== 'queued' || operation.laneRef !== binding.laneRef || operation.callerRootRef !== binding.callerRootRef || operation.projectRef !== binding.projectRef || operation.boardRef !== binding.boardRef) continue
      void enqueue(binding, operation).catch(() => undefined)
    }
  }
  ...
  const serializeReconciliation = (binding, operation, task) => {
    const key = operationRefKey(binding.laneRef, operation.operationRef), previous = reconciliationChains.get(key) || Promise.resolve()
    const current = previous.catch(() => undefined).then(task)
    reconciliationChains.set(key, current)
    return current.finally(() => { if (reconciliationChains.get(key) === current) reconciliationChains.delete(key) })
  }
  const reconcile = (binding, operation) => {
    const pending = pendingByOperation.get(operationRefKey(binding.laneRef, operation.operationRef))
    if (pending) return pending
    if (operation.state === 'queued') return enqueue(binding, operation)
    return serializeReconciliation(binding, operation, async () => {
      if (operation.state === 'ready' || operation.state === 'failed' || operation.state === 'cancelled') return publicOperation(binding, operation)
      const observed = { state: operation.state, phase: operation.phase, revision: Number.isSafeInteger(operation.revision) ? operation.revision : 1 }
      if (operation.phase === 'prompt_dispatched') {
        const evidence = await inspectRuntimePrompt({ sessionId: operation.sessionId, requestId: operation.promptRequestId })
        if (evidence && evidence.rpcId === operation.promptRequestId && typeof evidence.delivered === 'boolean' && ['session/control', 'session/follow'].includes(evidence.source)) {
          if (evidence.delivered) { operation.phase = 'prompted'; operation.state = 'ready'; delete operation.errorCode }
          else { operation.phase = 'renamed'; operation.state = 'failed'; operation.errorCode = 'HOST_SESSION_PROMPT_NOT_DELIVERED' }
          operation.revision = observed.revision + 1; operation.updatedAt = Date.now(); await persist(true)
          return publicOperation(binding, operation)
        }
      }
      let sessions
      try {
        sessions = await callRuntimeRpc('session/list', { args: { _request: {} } })
        ...
        if (TERMINAL.has(operation.state) || operation.state !== observed.state || operation.phase !== observed.phase || revision !== observed.revision) return publicOperation(binding, operation)
        operation.state = 'outcome_unknown'; operation.errorCode = 'HOST_SESSION_LAUNCH_OUTCOME_UNKNOWN'; operation.updatedAt = Date.now(); await persist()
        return publicOperation(binding, operation)
      }
  ...
  const resolveUnknown = (binding, operation, request) => serializeReconciliation(binding, operation, async () => {
  ...
  const close = () => {
    ...
    closed = true
    closePromise = (async () => {
      capabilityToken.fill(0); callerSalt?.fill(0)
      for (const queue of queues.values()) for (const item of queue) { item.operation.state = 'cancelled'; item.operation.phase = 'cancelled'; item.operation.updatedAt = Date.now(); item.resolve(publicOperation(item.binding, item.operation)) }
      queues.clear(); projectOrder.length = 0
      const active = server; server = undefined
      const serverClosed = active ? new Promise(resolve => active.close(resolve)) : Promise.resolve()
      for (const socket of acceptedSockets) socket.destroy()
      await persist(true).catch(() => undefined)
      await Promise.allSettled([...acceptedRequests])
      await Promise.allSettled([...activeJobs, ...reconciliationChains.values()])
      await writeChain.catch(() => undefined)
      await serverClosed
      pendingByOperation.clear(); reconciliationChains.clear(); acceptedSockets.clear()
      if (process.platform !== 'win32') await rm(endpoint, { force: true }).catch(() => undefined)
    })()
    return closePromise
  }
  const diagnostics = () => ({ running, queued: [...queues.values()].reduce((sum, queue) => sum + queue.length, 0), activeProjectLanes: projectOrder.length, acceptedSockets: acceptedSockets.size, records: document?.operations.length || 0, terminalRecords: document?.operations.filter(row => TERMINAL.has(row.state)).length || 0, bindingIndexSize: bindingsByCaller.size, ticketIndexSize: bindingsByTicket.size, operationIndexSize: operationsByRef.size, operationSlotIndexSize: operationsBySlot.size })
