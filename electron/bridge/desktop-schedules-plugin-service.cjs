...
async function ensurePatchEntries(file) {
  ...
  const additions = []
  if (!entries.some(item => item?.id === SCHEDULE_ID || item?.name === SCHEDULE_PACKAGE)) additions.push({ id: SCHEDULE_ID, name: SCHEDULE_PACKAGE })
  if (!entries.some(item => item?.id === PLUGIN_ID || item?.name === PLUGIN_PACKAGE)) additions.push({ id: PLUGIN_ID, name: PLUGIN_PACKAGE })
  if (!additions.length) return false
  if (rows == null) document.contents = document.createNode([])
  document.contents.flow = false
  document.add({ insert: additions })
  await atomicWrite(file, String(document))
  return true
}

async function ensureDesktopSchedulesPlugin({ dshHome, bundledRoot }) {
  ...
  if (manifest.name !== PLUGIN_PACKAGE) throw new Error('内置定时任务插件包无效。')
