...
app.whenReady().then(async () => {
  ...
  for (let step = 0; step < 120; step += 1) {
    ...
    await sleep(25)
    focus = await evaluate(window, \`(()=>{const node=document.activeElement;const name=(node?.getAttribute('aria-label')||node?.innerText||'').trim();const rect=node?.getBoundingClientRect();const style=node?getComputedStyle(node):null;const visibleColor=value=>!!value&&value!=='transparent'&&!/rgba\\([^)]*,\\s*0(?:\\.0+)?\\s*\\)/i.test(value);const outlineVisible=!!style&&style.outlineStyle!=='none'&&parseFloat(style.outlineWidth)>0&&visibleColor(style.outlineColor);const shadowVisible=!!style&&style.boxShadow!=='none'&&visibleColor(style.boxShadow);const point=rect?[rect.left+rect.width/2,rect.top+rect.height/2]:null;const hit=point?document.elementFromPoint(point[0],point[1]):null;const unobscured=!!node&&!!hit&&(hit===node||node.contains(hit));return {name,tag:node?.tagName,role:node?.getAttribute('role'),className:node?.className,focused:!!node?.matches?.(':focus'),focusVisible:!!node?.matches?.(':focus-visible'),nativeOrAria:node?.tagName==='BUTTON'||node?.getAttribute('role')==='button',outlineStyle:style?.outlineStyle,outlineWidth:style?.outlineWidth,outlineColor:style?.outlineColor,boxShadow:style?.boxShadow,outlineVisible,shadowVisible,rect:rect&&{left:rect.left,top:rect.top,right:rect.right,bottom:rect.bottom,width:rect.width,height:rect.height},inViewport:!!rect&&rect.left>=0&&rect.top>=0&&rect.right<=innerWidth&&rect.bottom<=innerHeight,ring:outlineVisible||shadowVisible,unobscured}})()\`)
