...
async function evaluate(window, source, stage = 'renderer evaluation') {
  try { return await window.webContents.executeJavaScript(source, true) }
  catch (error) { throw new Error(stage + ': ' + String(error?.stack || error)) }
}
...
app.whenReady().then(async () => {
  ...
  const baseline = await evaluate(window, \`(()=>{const node=[...document.querySelectorAll('button,[role=button]')].find(node=>/生成继续请求|准备继续请求|prepare continue request|resume/i.test((node.innerText||node.getAttribute('aria-label')||'').trim()));const style=node?getComputedStyle(node):null;return {outlineStyle:style?.outlineStyle,outlineWidth:style?.outlineWidth,outlineColor:style?.outlineColor,boxShadow:style?.boxShadow}})()\`, 'read Resume focus baseline')
  const desktop = await evaluate(window, \`(()=>{const root=document.querySelector('.dat-workspace');const text=root?.innerText||'';const buttons=root?[...root.querySelectorAll('button')]:[];const resume=buttons.filter(node=>/生成继续请求|准备继续请求|prepare continue request|resume/i.test((node.innerText||node.getAttribute('aria-label')||'')));const stop=buttons.filter(node=>/停止团队|stop team/i.test((node.innerText||node.getAttribute('aria-label')||'')));return {viewportSize:[innerWidth,innerHeight],rendered:!!root,paused:/已由用户停止|stopped by (?:the )?user/i.test(text),resumeCount:resume.length,stopCount:stop.length,agentDirectory:/代理目录|agent directory|统一代理目录|unified agent catalog|查看团队关系|view team relationships?/i.test(text)}})()\`, 'read desktop Agent Teams workbench')
  await setViewport(window, 390, 844)
  await sleep(500)
  const mobile = await evaluate(window, \`(()=>{const root=document.querySelector('.dat-workspace');const rect=root?.getBoundingClientRect();const viewport=document.querySelector('meta[name=viewport]')?.content||'';const targets=root?[...root.querySelectorAll('button')].filter(node=>/生成继续请求|准备继续请求|prepare continue request|resume|代理目录|agent directory|统一代理目录|unified agent catalog|查看团队关系|view team relationships?/i.test((node.innerText||node.getAttribute('aria-label')||''))).map(node=>{const r=node.getBoundingClientRect();return {name:(node.innerText||node.getAttribute('aria-label')||'').trim(),width:r.width,height:r.height}}):[];return {viewportSize:[innerWidth,innerHeight],viewportMeta:/width=device-width/i.test(viewport),rendered:!!root,visible:!!rect&&rect.width>0&&rect.height>0,noHorizontalOverflow:document.documentElement.scrollWidth<=document.documentElement.clientWidth,touchTargets:targets,touchTargets44:targets.length>0&&targets.every(target=>target.width>=44&&target.height>=44)}})()\`, 'read mobile Agent Teams workbench')
  await setViewport(window, 1280, 900)
  await sleep(500)
  window.show()
  window.focus()
  ...
  for (let attempt = 0; attempt < 40; attempt += 1) {
    const value = await evaluate(window, \`(()=>{const input=document.querySelector('[data-composer-input]');return (input?.innerText||input?.textContent||'').trim()})()\`, 'read Chat composer draft')
    if (/team_resume|恢复|继续|continue|resume/i.test(value)) break
    await sleep(50)
  }
  const afterDraft = await evaluate(window, \`(()=>{const root=document.querySelector('.dat-workspace');const rootRect=root?.getBoundingClientRect();const rootStyle=root?getComputedStyle(root):null;const input=document.querySelector('[data-composer-input]');const inputRect=input?.getBoundingClientRect();const inputStyle=input?getComputedStyle(input):null;const inputValue=(input?.innerText||input?.textContent||'').trim();const workbenchInactive=!root||!rootRect||rootRect.width<=0||rootRect.height<=0||rootStyle?.display==='none'||rootStyle?.visibility==='hidden';const composerVisible=!!input&&!!inputRect&&inputRect.width>0&&inputRect.height>0&&inputStyle?.display!=='none'&&inputStyle?.visibility!=='hidden';return {viewportSize:[innerWidth,innerHeight],workbenchInactive,composerMounted:!!input,composerVisible,preview:/team_resume|恢复|继续|continue|resume/i.test(inputValue),inputValue}})()\`, 'read post-draft Chat view')
  if (window.webContents.debugger.isAttached()) window.webContents.debugger.detach()
  fs.writeFileSync(out, JSON.stringify({ desktop, focusBaseline: baseline, focus, mobile, afterDraft }))
  app.exit(0)
...
`
  ...
  if (!report.desktop?.rendered || !report.desktop.paused || report.desktop.resumeCount < 1 || report.desktop.stopCount !== 0 || !report.desktop.agentDirectory) throw new Error(`Paused/Resume DOM interaction failed: ${JSON.stringify(report.desktop)}`)
  ...
  if (report.afterDraft?.viewportSize?.[0] !== 1280 || report.afterDraft?.viewportSize?.[1] !== 900 || !report.afterDraft.workbenchInactive || !report.afterDraft.composerMounted || !report.afterDraft.composerVisible || !report.afterDraft.preview) throw new Error(`Resume draft did not switch to a visible Chat composer: ${JSON.stringify(report.afterDraft)}`)
  return { desktop: true, desktopViewport: 'Electron Chromium 1280x900', mobileViewport: 'Electron Chromium 390x844 (not Android/iOS hardware)', keyboardAndAria: true, focusStyleChanged: true, focusedRectInViewport: true, visibleFocusUnobscured: true, touchTargets44: true, stopResumePreview: true, draftSwitchToChat: true, unifiedAgentDirectory: true }
}
