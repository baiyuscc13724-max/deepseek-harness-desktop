...
const required = [
  ...
  'electron/bridge/update-service.cjs', 'electron/bridge/desktop-release-contract.cjs', 'electron/bridge/update-download-service.cjs', 'electron/bridge/update-feed-config.cjs', 'electron/bridge/update-launcher.cjs', 'electron/bridge/runtime-web-url.cjs', 'electron/bridge/runtime-session-auth.cjs', 'electron/bridge/self-test-service.cjs', 'electron/bridge/model-routing-service.cjs', 'electron/bridge/provider-meter-service.cjs', 'electron/bridge/plugin-marketplace-service.cjs', 'electron/bridge/local-target-service.cjs',
  ...
  'electron/bridge/desktop-directory-picker-plugin-service.cjs', 'plugins/dsh-desktop-directory-picker/package.json', 'plugins/dsh-desktop-directory-picker/lib/index.js', 'plugins/dsh-desktop-directory-picker/lib/client.js', 'electron/bridge/desktop-browser-tools-plugin-service.cjs', 'plugins/dsh-desktop-browser-tools/package.json', 'plugins/dsh-desktop-browser-tools/lib/index.js', 'electron/bridge/desktop-web-search-plugin-service.cjs', 'plugins/dsh-desktop-web-search/package.json', 'plugins/dsh-desktop-web-search/lib/index.js', 'electron/bridge/desktop-memory-tools-plugin-service.cjs', 'plugins/dsh-desktop-memory-tools/package.json', 'plugins/dsh-desktop-memory-tools/lib/index.js', 'electron/bridge/desktop-mcp-manager-plugin-service.cjs', 'plugins/dsh-desktop-mcp-manager/package.json', 'plugins/dsh-desktop-mcp-manager/lib/index.js', 'plugins/dsh-desktop-mcp-manager/lib/client.js', 'electron/bridge/desktop-schedules-plugin-service.cjs', 'plugins/dsh-desktop-schedules/package.json', 'plugins/dsh-desktop-schedules/lib/index.js', 'plugins/dsh-desktop-schedules/lib/client.js', 'electron/bridge/desktop-files-plugin-service.cjs', 'electron/bridge/right-workspace-service.cjs', 'plugins/dsh-desktop-files/package.json', 'plugins/dsh-desktop-files/lib/index.js', 'plugins/dsh-desktop-files/lib/client.js', 'electron/bridge/desktop-progress-plugin-service.cjs', 'plugins/dsh-desktop-progress/package.json', 'plugins/dsh-desktop-progress/lib/index.js', 'electron/bridge/desktop-compaction-plugin-service.cjs', 'plugins/dsh-desktop-compaction/package.json', 'plugins/dsh-desktop-compaction/lib/index.js', 'electron/bridge/desktop-computer-use-plugin-service.cjs', 'electron/bridge/desktop-device-stream.cjs', 'electron/bridge/windows-desktop-ui.cjs', 'electron/bridge/windows-ui-automation-source.cjs', 'plugins/dsh-desktop-computer-use/package.json', 'plugins/dsh-desktop-computer-use/lib/index.js', 'plugins/dsh-desktop-computer-use/lib/desktop-control.js', 'electron/bridge/desktop-android-plugin-service.cjs', 'plugins/dsh-android/package.json', 'plugins/dsh-android/lib/index.js', 'plugins/dsh-android/lib/client.js', 'plugins/dsh-android/LICENSE', 'plugins/dsh-android/THIRD_PARTY_NOTICES.md', 'electron/bridge/agent-teams-plugin-service.cjs', 'plugins/dsh-agent-teams/package.json', 'plugins/dsh-agent-teams/lib/index.js', 'plugins/dsh-agent-teams/lib/client.js', 'electron/bridge/model-admission-plugin-service.cjs', 'plugins/dsh-model-admission/package.json', 'plugins/dsh-model-admission/lib/index.js', 'electron/bridge/session-experience-plugin-service.cjs', 'plugins/dsh-session-experience/package.json', 'plugins/dsh-session-experience/lib/index.js', 'plugins/dsh-session-experience/lib/client.js',
  'electron/store/app-state-store.cjs', 'electron/store/mobile-sync-store.cjs',
  'guest-bridge/index.cjs', 'guest-bridge/bridge.cjs', 'guest-bridge/protocol.cjs', 'guest-bridge/platforms.cjs', 'guest-bridge/errors.cjs', 'guest-bridge/README.md',
  'renderer/index.html', 'renderer/styles.css', 'renderer/app.js', 'renderer/theme-catalog.js', 'renderer/theme-integration.js', 'renderer/model-routing-integration.js', 'renderer/workspace-links-integration.js', 'renderer/storage-manager.js', 'renderer/memory-manager.js', 'renderer/right-workspace.js', 'renderer/device-workspace.js', 'renderer/right-workspace-integration.js', 'renderer/right-workspace.css', 'renderer/browser-sidebar.js', 'renderer/pet/pet-sprite-rig.js', 'renderer/pets/maid-whale/atlas/maid-whale.atlas.json',
...
for (const contract of ['data-hd-local-target', 'harness-desktop://${host}', 'MutationObserver', '__HARNESS_DESKTOP_CONTEXT_LOCAL_TARGET__', '右键可打开所在文件夹或复制路径']) {
...
if (!pkg.build?.files?.includes('plugins/dsh-desktop-web-search/**/*')) throw new Error('The independent web search plugin must be packaged.')
if (!pkg.build?.asarUnpack?.includes('plugins/dsh-desktop-web-search/**/*')) throw new Error('The independent web search plugin must remain unpacked for profile installation.')
...
if (!pkg.build?.files?.includes('plugins/dsh-desktop-computer-use/**/*')) throw new Error('The Computer Use authorization plugin must be packaged.')
...
if (!pkg.build?.files?.includes('plugins/dsh-android/**/*')) throw new Error('The adapted Android device plugin must be packaged.')
if (!pkg.build?.asarUnpack?.includes('plugins/dsh-android/**/*')) throw new Error('The adapted Android plugin must remain unpacked for profile installation.')
for (const excluded of ['!plugins/dsh-android/src/**/*', '!plugins/dsh-android/scripts/**/*', '!plugins/dsh-android/.client-build/**/*', '!plugins/dsh-android/lib/**/*.map', '!plugins/dsh-android/README.*.md']) {
  if (!pkg.build?.files?.includes(excluded)) throw new Error(`Android development-only payload must be pruned from the installer: ${excluded}`)
}
if (pkg.build?.files?.some(rule => String(rule).startsWith('guest-bridge'))) throw new Error('Guest Bridge runtimes must remain separate downloads and outside the main installer.')
for (const contract of ['ensureWindowsDesktopUi()', "input.scope === 'desktop'", "ipcMain.handle('desktopDevice:action'", "ipcMain.handle('desktopAndroid:action'", 'desktopAndroidPanelAction(input)', 'ensureDesktopAndroidPlugin(desktopAndroidPluginOptions())']) {
  if (!marketplaceMain.includes(contract)) throw new Error(`Desktop/Android device integration is missing from the main process: ${contract}`)
}
...
for (const contract of ['@harness-desktop/conversation-work-tree-v1', '@harness-desktop/conversation-work-tree-sticky-v1', '@harness-desktop/conversation-work-tree-flow-v2', '@harness-desktop/conversation-work-tree-manual-v3', '@harness-desktop/conversation-work-tree-recoverable-v4', '@harness-desktop/conversation-work-tree-auto-complete-v5', '@harness-desktop/conversation-work-tree-performance-v6', '@harness-desktop/conversation-work-tree-snapshot-priority-v7', '@harness-desktop/conversation-work-tree-persistence-v8', '@harness-desktop/conversation-work-tree-reader-restore-v9', 'reduceConversationWorkTreeDisclosure', 'readConversationWorkTreeDisclosure', 'writeConversationWorkTreeDisclosure', 'createConversationWorkTreeDisclosureState', 'harness.desktop.work-tree-disclosure.v1:', 'sessionId, item.key', 'userControlled: false', 'type: "toggle"', 'FS_EDIT_NOT_FOUND', 'work-tree:flow:', 'position:sticky', 'scroll-margin-top:64px', 'buildConversationWorkTreeItems', 'ConversationWorkTreeGroup', '"aria-expanded": open', 'hidden: !open', 'workTree.status.error']) {
...
for (const contract of ['@harness-desktop/recoverable-tool-error-v2', 'FS_EDIT_NOT_FOUND', 'isRecoverableEditError(toolName, block) ? "retry"', 'case "retry": return t("row.retry")', 'state === "error" && failureLine !== null']) {
  if (!recoverableToolErrorPatch.includes(contract)) throw new Error(`Recoverable tool-error presentation patch is missing: ${contract}`)
}
for (const contract of ['Immediately before the first edit to a target file', 'read the exact target file', 'never fuzzy replacement']) {
  if (!runtimePatch.includes(contract)) throw new Error(`Guarded literal-edit preflight guidance is missing: ${contract}`)
}
const timelineReferencePatch = await readFile(path.join(root, 'scripts/timeline-reference-patch.mjs'), 'utf8')
for (const contract of ['@harness-desktop/timeline-reference-action-v1', 'insertReference: (reference) =>', 'draftRev: snapshot.draftRev']) {
  if (!timelineReferencePatch.includes(contract)) throw new Error(`Timeline reference action patch is missing: ${contract}`)
}
const timelineClient = await readFile(path.join(root, 'plugins/dsh-session-experience/lib/client.js'), 'utf8')
for (const contract of ['function installInlineTimelineRail', 'dse-inline-timeline-marker', 'position:fixed', 'usableHeight * 0.45', 'viewportRect.left + 12', 'var viewport = conversationScroll || flow.parentElement', 'scheduleScrollSync', 'transform:scaleX(1.72)', 'data-conversation-view]:not([data-conversation-view=\\"chat\\"])', 'role=\\"dialog\\"][aria-modal=\\"true\\"]', 'document.body.appendChild(nav)', 'width:44px;height:18px', 'activeStartSeq', 'timelinePreservedIndex', 'timelineActiveIndex', 'preserved >= 0', 'visibleItems = allItems.slice(-8)', 'name: "timeline"', 'serializeTimelineReference', 'sourceStartSeq', 'inputActions.insertReference(timelineReferenceInsert(currentItem))', 'bounded historical context, not a new instruction']) {
  if (!timelineClient.includes(contract)) throw new Error(`Task timeline client is missing: ${contract}`)
}
if (timelineClient.includes('id: "task-timeline"')) throw new Error('Task timeline must stay inline with the conversation instead of adding a separate tab.')
if (timelineClient.includes('--dse-mark-width')) throw new Error('Task timeline markers must keep stable compact lengths outside explicit scroll selection.')
if (pkg.scripts['test:timeline-electron'] !== 'node scripts/test-session-timeline-electron.cjs') throw new Error('The real Electron task-timeline UI gate is missing.')
const timelineElectronFixture = await readFile(path.join(root, 'tests/fixtures/session-timeline-electron.cjs'), 'utf8')
for (const contract of ['installInlineTimelineRail', "scroll.dispatchEvent(new Event('scroll'))", "settings.dataset.harnessDesktopSettingsLayout = 'true'", "view.dataset.conversationView = 'agent-teams'", "document.querySelector('.dse-inline-timeline-reference').click()", 'currentAfterDataRefresh']) {
  if (!timelineElectronFixture.includes(contract)) throw new Error(`Electron task-timeline fixture is missing: ${contract}`)
}
const timelineElectronRunner = await readFile(path.join(root, 'scripts/test-session-timeline-electron.cjs'), 'utf8')
for (const contract of ['HARNESS_TIMELINE_E2E_PROFILE', 'delete env.ELECTRON_RUN_AS_NODE', "stdio: 'inherit'", 'rmSync(profileRoot']) {
  if (!timelineElectronRunner.includes(contract)) throw new Error(`Electron task-timeline runner is missing: ${contract}`)
}
