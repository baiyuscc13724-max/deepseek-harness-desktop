...
for (const contract of [`component-publish/${releaseTag}`, 'fetch-depth: 0', 'HARNESS_COMPONENT_SIGNING_PRIVATE_KEY_BASE64', 'base64 --decode', "trap 'rm -f", 'prepare-production-components.mjs', 'verify-production-component-staging.mjs', 'Preserve matching assets and identify missing component assets', 'gh release upload', 'Re-download and verify public component assets', 'Sign exact desktop release manifest in protected CI', 'refresh-release-manifest.mjs', 'branch="release-manifest/$RELEASE_TAG"', 'refs/tags/$RELEASE_TAG', "git rev-parse 'FETCH_HEAD^{}'", 'main_revision="$(git rev-parse HEAD)"', 'git merge-base --is-ancestor', 'git diff-tree --no-commit-id --name-only -r', 'cmp "$manifest_file"', 'git reset --hard HEAD', 'git clean -fd', "GITHUB_EVENT_NAME", "refs/heads/main", 'release: refresh $RELEASE_TAG signed manifest', 'git push origin "HEAD:refs/heads/main"', 'git push origin "HEAD:refs/heads/$branch"']) {
