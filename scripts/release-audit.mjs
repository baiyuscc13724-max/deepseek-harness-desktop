...
for (const contract of ['cnb-cloud-release-', 'get-build-status', 'CNB metadata pushed', 'Method Head', 'SHA256SUMS.txt', 'COMPONENT-SHA256SUMS.txt', 'linux-x86_64.AppImage', 'mac-arm64.dmg', 'mac-x64.dmg', 'android-universal.apk.sha256', 'desktop-shell-', 'components-', 'component-feeds/stable/win32-x64.json', 'component-feeds/stable/darwin-x64.json', 'component-feeds/stable/darwin-arm64.json', 'Stable component feeds must be absent or complete', 'Stable-only CNB synchronization requires the verified SHA256SUMS.txt', 'Manifest SHA-256 missing', 'does not match the public GitHub release asset', 'GitHub source verification failed', "credential.helper='"]) {
...
for (const contract of ['repos/${repo}/actions/runs/${runId}', 'workflowPath', 'WORKFLOWS.android', 'WORKFLOWS.components', 'WORKFLOWS.recovery', 'validateCompletedPhaseEvidence', 'verifyCloudAssetMirrorsBeforeStable', 'validateGithubReleaseAgainstManifest']) {
  if (!releasePublisher.includes(contract)) throw new Error(`Publisher run identity/stable-last contract missing: ${contract}`)
}
for (const contract of ["delete state.phases['local-windows']", 'incorrectlyMigratedLocal', 'delete state.phases[localGatePhase]', 'matchesWorkflowRunIdentity', 'validateCompletedPhaseEvidence', 'validateGithubReleaseAgainstManifest', 'validateCnbMirrorObservations']) {
  if (!releasePublisherSelection.includes(contract)) throw new Error(`Publisher fail-closed behavior helper missing: ${contract}`)
}
...
for (const contract of ['local-mobile-gates', 'immutable-mobile-tag', 'github-signed-android', 'const workflowRef = mobile.tag', "'--ref', workflowRef", 'cnb-mobile-assets', 'maybeRebindPreTagCandidate', 'safe-pre-tag-main-fast-forward', 'Cannot prove CNB Android side-effect absence', 'POST_TAG_CONTROLLER_PATHS', 'assertPostTagControllerAdvance', 'const controllerCi = await waitForAndroidCiEvidence(repo, currentHead, pollSeconds)', 'controllerCiRunId', 'assertRecoverableAndroidPreflightFailure', 'releaseRecoveryAuthorization', 'createEmptyStandaloneRelease', 'assertStandaloneCnbAssetsAbsent', 'failedRequests.length >= 5', 'waitForAndroidCiEvidence', 'Android mobile compile/test', "'--workflow', 'ci.yml'", 'ensureAppleMobileEvidence', "'mobile_only=true'", 'iPhone and iPad simulators', 'macOS Desktop package contracts', 'verifyProtectedState', 'assertProtectedMetadataMatchesLocal', 'gitCaptureRaw', "'cat-file', 'blob', `HEAD:${file}`", 'Protected GitHub/CNB metadata differs from the committed source', 'assertProtectedReleaseMatchesManifest', 'GitHub asset evidence changed', 'GitHub release metadata differs from the reviewed manifest', 'release target differs from its immutable tag', 'tagRevision: remoteTagRevision', 'latestTag: readLatestGithubReleaseTag', 'Protected desktop latest release must remain', 'verifyProtectedCnbAssetBytes', 'Protected CNB asset bytes changed', 'verifyCnbBytes: true', 'release:cnb-mobile-cloud', '--scope android']) {
