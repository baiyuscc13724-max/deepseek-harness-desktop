...
function apply(ctx) {
  ctx.systemPrompt.section({
    ...
    order: 116,
    text: 'Computer Use is a built-in desktop skill; never ask the user to install it or add a skill card. Call status first. When activationRequired is true, call requestAuthorization so Harness Desktop pushes an authorization card above the dialog with “本次授权” and “永久授权” buttons; do not send the user to the Profile panel. The same grant and enabled state are shared with the right-sidebar browser_control, so the user authorizes only once. After authorization, unlimited is true for desktop Computer Use: click, type, and scroll execute without per-action confirmation, persistent application policy no longer gates targets, and UAC/system/elevated/sensitive-window and sensitive-input restrictions are bypassed; browser_control keeps its own credential, payment, and transaction safety boundaries. Use targets to discover visible Windows applications and select to bind one exact window. Screenshot reads the selected surface; for external applications, take a new screenshot before input because each input invalidates the prior surface. A session grant lasts until the app exits; a permanent grant survives restart. Stop ends the shared live control session and clears the target; no Shell or scripts are exposed.'
  })
  ctx.tools.register(defineTool({
    name: 'computer_use',
    description: 'Request one session or permanent Harness Desktop authorization shared with right-sidebar browser_control, then use unlimited desktop control across visible Windows applications. Authorized desktop input needs no per-action confirmation and bypasses application, UAC/system/elevated-window, and sensitive-input gates; browser_control keeps its own sensitive-data boundaries, and no Shell or scripts are exposed.',
    timeoutMs: 30000,
    parameters: {
