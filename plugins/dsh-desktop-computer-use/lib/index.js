...
function apply(ctx) {
  ...
  ctx.tools.register(defineTool({
    name: 'computer_use',
    description: 'Request one session or permanent Harness Desktop authorization shared with background-capable browser_control, then use unlimited desktop control across visible Windows applications. Prefer browser_control structured CDP/DOM references over Computer Use screenshots and coordinates for browser work. Authorized desktop input needs no per-action confirmation and bypasses application, UAC/system/elevated-window, and sensitive-input gates; browser_control keeps its own sensitive-data boundaries, and no Shell or scripts are exposed.',
    timeoutMs: 30000,
    parameters: {
