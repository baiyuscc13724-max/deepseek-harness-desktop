...
> Modified by Harness Desktop Contributors from the Apache-2.0 OpenAI `openai-docs` reference. The Codex-only model-resolution script was replaced with official-domain Harness web lookup.

Use this guide when the user explicitly asks to upgrade an existing integration to GPT-5.5. Pair it with current OpenAI docs lookups. The default target string is `gpt-5.5` only after the live docs confirm that target or when the user explicitly named it.

## Freshness check

Before applying this bundled guide for a latest/current/default model upgrade, use `web_search` with official-domain queries for the live latest-model, migration, and prompting guides. Open the relevant official pages with `browser_control` only when its status confirms the right-sidebar browser is available.

- If current official docs still identify GPT-5.5 as the target, continue with this bundled guide and use `references/prompting-guide.md` when prompt updates are needed.
- If current official docs identify a different model, use those live migration and prompting pages instead of this bundled reference.
- If remote verification fails, continue only as a disclosed fallback and say the bundled material may have drifted.
