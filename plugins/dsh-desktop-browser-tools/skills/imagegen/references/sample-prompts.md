...
These prompt recipes apply to the native Harness `image_gen` tool. Use them as starting points; they are intentionally complete recipes, not permission to add unrequested story elements.
...
The labeled lines are prompt scaffolding, not a closed tool schema. `Asset type` and `Input images` are prompt-only language, not dedicated Harness arguments.

Do not add unsupported controls such as `quality`, `input_fidelity`, masks, output formats, output paths, or model names to a Harness `image_gen` call.
