...
> Modified by Harness Desktop Contributors from the Apache-2.0 OpenAI `imagegen` reference. Codex-only CLI fallback instructions were removed.

These principles apply to the native Harness `image_gen` tool. The tool accepts only its declared `mode`, `prompt`, `size`, and optional workspace-contained `input_image_path` fields.
...
- [Harness tool execution controls](#harness-tool-execution-controls)
...
## Harness tool execution controls
- `quality`, `input_fidelity`, explicit masks, output format, output paths, model names, and arbitrary reference-image arrays are not Harness `image_gen` arguments.
- Use only the declared tool schema. If a requested control is unavailable, explain the limitation instead of inventing an argument or falling back to a script.
