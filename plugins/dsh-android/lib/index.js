...
// ── public API ───────────────────────────────────────────────────────────────
export { AdbError, AdbToolchain, DEFAULT_BOOT_TIMEOUT_MS, SERIAL_PATTERN, bootAvd, listAvds, resolveAdbBinary, resolveEmulatorBinary, } from './adb.js';
export { AdbFrameLoop, MultipartFrameWriter, PngFrameSplitter, STREAM_BOUNDARY, pngDimensions, } from './frame-source.js';
export { ANDROID_BUTTONS, ANDROID_DEVICE_ACTIONS, DEFAULT_ANDROID_STREAM_SCOPE, AndroidHostController, AndroidHostRegistry, NON_ASCII_TYPE_HINT, ROTATION_CYCLE, escapeInputText, isAndroidDeviceAction, isInputTextSafe, } from './android-host.js';
export { StreamAccessController, TOKEN_TTL_MS, classifyScreenshotPath, isLoopbackRemoteAddress, isTrustedRequest, openVerifiedScreenshot, prepareStreamAccessKey, screenshotDir, stateRoot, } from './stream-access.js';
export { CAPTURE_ROUTE_PATH, CONTROL_ROUTE_PATH, DEVICES_ROUTE_PATH, DEVICE_ACTION_ROUTE_PATH, GRANT_ROUTE_PATH, PLUGIN_ROUTE_PREFIX, SCREENSHOT_ROUTE_PREFIX, STATUS_ROUTE_PATH, STREAM_ROUTE_PREFIX, SWITCH_DEVICE_ROUTE_PATH, StreamRoutes, deviceStateErrorCode, installStreamRoutes, mountStreamRoutes, nextCapturePath, } from './stream-routes.js';
export { ANDROID_LABEL_HINT, filterAndroidApps, listAndroidApps, noMatchCandidateLines, noMatchListingHint, parseDumpsysPackageVersions, parsePmListPackages, resolveAppByName, } from './app-list.js';
export { APK_SEARCH_SKIPPED_DIRECTORIES, applicationIdFromOutputMetadata, applicationIdFromSources, assembleGradleArgs, assembleTask, buildFailureDetail, buildRun, detectProject, filterBuildOutput, findBuiltApk, isApkFile, launchPackage, moduleDirectoryOf, resolveApplicationId, runGradleBuild, } from './build-run.js';
/**
...
export { ANDROID_UI_TOOL_NAMES, createAndroidUiTools, } from './tool-uitree.js';
export { ANDROID_OCR_TOOL_NAMES, createAndroidOcrTools, } from './tool-ocr.js';
export { ANDROID_ROW_TOOL_NAMES, createAndroidRowTools, } from './tool-list-rows.js';
export { LOG_BUFFERS, LOG_PRIORITIES, LogLineRing, MAX_LOG_BYTES, MAX_LOG_LINES, compileGrep, createAndroidLogTools, deviceStartTimestamp, durationSeconds, followSeconds, postProcess, resolvePackagePid, runLogCapture, snapshotDuration, } from './tool-logs.js';
export { ANDROID_DEBUG_TOOL_NAMES, capBacktrace, createAndroidDebugTools, parseMeminfo, parsePackageInfo, parseProcessTable, } from './tool-debug.js';
...
export { IMAGE_REF_SCHEMA, imageInputActive, renderJsonWithImage, resolveVisionServices, saveScreenshotAttachment, } from './vision.js';
...
export function apply(ctx) {
    const hostCtx = ctx;
    const hosts = new AndroidHostRegistry();
    // Non-streaming tool families keep the legacy default controller; core
    // stream tools and web routes resolve a conversation-owned controller.
    const host = hosts.hostFor();
    ...
    const tools = createAndroidTools(hosts, { vision });
    ...
    installStreamRoutes(ctx, hosts);
    const adb = hosts.toolchain.binary;
    ctx.logger.info(`dsh-android mounted (${ANDROID_TOOL_NAMES.join(' + ')} + ${ANDROID_UI_TOOL_NAMES.join(' + ')} + `
