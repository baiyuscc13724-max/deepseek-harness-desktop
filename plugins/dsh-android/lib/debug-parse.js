...
export function parseProcessTable(stdout) {
    ...
    if (processes.length === 0) {
        throw new Error('could not parse the `ps -A` output — it carried no process rows; the listing FAILED, which is not '
    ...
    for (const rawLine of stdout.split('\n')) {
        ...
        if (inDetail) {
            ...
            if (row !== null) {
                ...
                if (name !== 'TOTAL' && Number.isSafeInteger(pssKb))
                    detail.push({ name, pssKb });
            }
            continue;
        }
        if (inSummary) {
            ...
            if (row !== null)
                summary[row[1].trim()] = Number(row[2]);
        }
    }
    const totalPssKb = totals === null ? undefined : Number(totals[1]);
    if (totalPssKb === undefined || !Number.isSafeInteger(totalPssKb))
        return undefined;
 ...
 */
export function parsePackageInfo(stdout, packageName) {
 ...
 */
export function capBacktrace(lines, limit = MAX_BACKTRACE_LINES) {
...
export function firstThreadBlock(lines) {
 ...
 */
export async function readNewestAnrTrace(host, serial, timeoutMs) {
    ...
    if (content.trim() === '' || /Permission denied/i.test(content))
        return undefined;
