...
export class PngFrameSplitter {
    ...
    #frameEnd() {
        ...
        for (;;) {
            ...
            if (dataLength > MAX_FRAME_BYTES)
                return undefined; // corrupt header; caller rescans
...
export class AdbFrameLoop {
    ...
    #frameWaiters = [];
    #stopped = false;
    constructor(serial, toolchain, events = {}) {
        this.serial = serial;
    ...
    start() {
        ...
        // Pace capture at ~2 fps. Unthrottled screencap reaches ~8 fps but can
        // saturate an emulator CPU and make the whole Harness GUI sluggish.
        const child = this.toolchain.spawnExecOut(this.serial, ['while :; do screencap -p; sleep 0.5; done']);
        this.#child = child;
        child.stdout?.on('data', (chunk) => {
            for (const png of this.#splitter.push(chunk))
                this.#acceptFrame(png);
        });
