...
import { DEBUG_MAX_BUFFER, MAX_BACKTRACE_LINES, capBacktrace, firstThreadBlock, parseMeminfo, parsePackageInfo, parseProcessTable, readNewestAnrTrace, } from './debug-parse.js';
...
export { capBacktrace, parseMeminfo, parsePackageInfo, parseProcessTable, } from './debug-parse.js';
...
const BACKTRACE_TRUNCATION_HINT = '[dsh-android: backtrace truncated at ~200 lines — pass all_threads:false '
...
export function createAndroidDebugTools(host, options = {}) {
    ...
    const target = async (tool, serial) => {
        if (!host.available) {
            throw new Error(`${tool}: adb is unavailable — ${host.toolchain.binary.reason ?? 'adb was not found'}. Install the `
                + 'Android SDK platform-tools, put adb on PATH, or set the ADB environment variable, then retry.');
        }
        if (disposed)
            throw new Error(`${tool}: the plugin is shutting down`);
        let device;
        try {
    ...
    const pidOf = async (serial, packageName) => {
    ...
    const androidBacktrace = defineTool({
        ...
        async execute(args) {
            ...
            if (pid === undefined && packageName === undefined) {
                throw new Error('android_backtrace: pass package_name (e.g. "com.example.app") or an explicit pid from '
            ...
            if (pid !== undefined) {
                ...
                if (killed !== undefined) {
                    ...
                    if (trace !== undefined) {
                        ...
                        if (truncated)
                            lines.push(BACKTRACE_TRUNCATION_HINT);
                        return {
                            device,
                            ...(pid === undefined ? {} : { pid }),
            ...
            const crash = await host.toolchain.shell(serial, [
                ...
                throw new Error(`android_backtrace: neither an ANR trace nor the crash buffer could be read on ${serial}: `
    ...
    const androidMeminfo = defineTool({
        ...
        async execute(args) {
            ...
            if (/No process found for/i.test(stdout)) {
                throw new Error(`android_meminfo: no running process for "${packageName}" on ${serial} — meminfo measures a LIVE `
            ...
            if (parsed === undefined) {
                throw new Error(`android_meminfo: \`dumpsys meminfo ${packageName}\` produced output without a TOTAL PSS line — `
            ...
            return { device, packageName, ...parsed };
        },
        presentCall: (args) => ({
            card: 'generic',
