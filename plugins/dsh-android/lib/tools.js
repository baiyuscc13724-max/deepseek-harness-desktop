...
import { bootAvd, listAvds } from './adb.js';
import { AndroidHostRegistry } from './android-host.js';
import { createAndroidAppTools } from './tool-apps.js';
import { INTERACT_ACTIONS, INTERACT_SETTLE_MS, SCROLL_DIRECTIONS, ScreenshotStore, assertAdbAvailable, captureScreenshot, deviceSchema, deviceSummary, errorMessage, performInteract, renderJson, resolveTarget, screenshotMeta, sleep, } from './tool-support.js';
...
/** Resolve the conversation-owned stream while keeping legacy single-host callers valid. */
function androidHostForExecution(host, exec) {
    if (!(host instanceof AndroidHostRegistry))
        return host;
    const id = exec?.agent?.id;
    return host.hostFor(typeof id === 'string' ? id : undefined);
}
/** Create the eight `android_*` core tool definitions bound to session-aware hosts. */
export function createAndroidTools(host, options = {}) {
    ...
    // capture would be un-grantable. A caller-supplied cacheDir keeps the same
    // `<cacheDir>/screenshots` layout.
    const screenshots = new ScreenshotStore(options.cacheDir === undefined ? screenshotDir() : join(cacheDir, 'screenshots'));
    const androidDevices = defineTool({
        ...
        async execute(_args, exec) {
            const activeHost = androidHostForExecution(host, exec);
            assertAdbAvailable(activeHost, 'android_devices');
            let listed;
            try {
                listed = await activeHost.toolchain.listDevices();
            }
            catch (error) {
                // A failed enumeration THROWS: an empty list must always be a fact
                // about the machine, never about adb having fallen over.
                throw new Error(`android_devices: adb could not enumerate devices: ${errorMessage(error)} — try \`adb kill-server\` `
                    + 'and retry, or check that the USB cable carries data');
            }
            const streamed = activeHost.streamedSerial;
            const devices = [];
            for (const device of listed) {
                ...
                const details = device.state === 'device'
                    ? await activeHost.toolchain.deviceDetails(device).catch(() => undefined)
                    : undefined;
                devices.push({
    ...
    const androidBoot = defineTool({
        ...
        async execute(args, exec) {
            const activeHost = androidHostForExecution(host, exec);
            assertAdbAvailable(activeHost, 'android_boot');
            ...
            const listed = await activeHost.toolchain.listDevices();
            let target = listed.find(device => device.serial === reference && device.state === 'device');
            let booted = false;
            if (target === undefined) {
                target = await bootReferencedAvd(activeHost, reference, listed);
                booted = true;
            }
            try {
                await activeHost.ensureStreaming({ serial: target.serial });
            }
            catch (error) {
                throw new Error(`android_boot: the live stream for ${target.serial} did not start: `
                    + `${errorMessage(error).replace(/^dsh-android: /, '')}`);
            }
            const details = await activeHost.toolchain.deviceDetails(target).catch(() => undefined);
            return {
    ...
    const androidShutdown = defineTool({
        ...
        async execute(args, exec) {
            const activeHost = androidHostForExecution(host, exec);
            const { device, summary } = await resolveTarget(activeHost, 'android_shutdown', args.device);
            if (!device.emulator) {
                throw new Error(`android_shutdown: ${device.serial} is a physical device — this tool powers off EMULATORS only. `
            ...
            if (host instanceof AndroidHostRegistry)
                await host.stopSerial(device.serial);
            else if (activeHost.streamedSerial === device.serial)
                await activeHost.stop();
            try {
                await activeHost.toolchain.exec(['emu', 'kill'], { serial: device.serial, timeoutMs: 30_000 });
            }
            catch (error) {
                throw new Error(`android_shutdown: \`adb -s ${device.serial} emu kill\` failed: ${errorMessage(error)} — the `
    ...
    const androidScreenshot = defineTool({
        ...
        async execute(args, exec) {
            const activeHost = androidHostForExecution(host, exec);
            const { device, summary } = await resolveTarget(activeHost, 'android_screenshot', args.device);
            return captureScreenshot(activeHost, screenshots, 'android_screenshot', device, summary, vision === undefined ? undefined : { services: vision, exec });
        },
        presentCall: (args) => ({
            card: 'generic',
    ...
    const androidInteract = defineTool({
        ...
        async execute(args, exec) {
            const activeHost = androidHostForExecution(host, exec);
            const { device, summary } = await resolveTarget(activeHost, 'android_interact', args.device);
            try {
                await performInteract(activeHost, device.serial, args);
            }
            catch (error) {
                const message = errorMessage(error);
                if (message.startsWith('android_interact:'))
                    throw error;
                throw new Error(`android_interact: ${args.action} on ${device.serial} failed: ${message.replace(/^dsh-android: /, '')}`);
            }
            ...
            const screenshot = await captureScreenshot(activeHost, screenshots, 'android_interact', device, summary, vision === undefined ? undefined : { services: vision, exec });
            return { action: args.action, ...screenshot };
        },
        presentCall: (args) => ({
            card: 'generic',
    ...
    const appTools = createAndroidAppTools(androidHostForExecution(host));
    return {
        androidDevices,
        androidBoot,
        androidShutdown,
 ...
 */
async function bootReferencedAvd(host, reference, listed) {
    const known = listed.find(device => device.serial === reference);
    const avds = await listAvds().catch(() => []);
    if (!avds.includes(reference)) {
        if (known !== undefined) {
            throw new Error(`android_boot: device ${reference} is ${known.state}, not ready to stream`
                + (known.state === 'unauthorized'
                    ? ' — accept the USB debugging prompt on the device, then retry'
                    : ' — wait for it to finish booting, then retry'));
        }
        throw new Error(`android_boot: "${reference}" is neither an online adb serial nor a known AVD name — run `
    ...
    try {
        ...
        throw new Error(`android_boot: booting the AVD "${reference}" failed: ${errorMessage(error).replace(/^dsh-android: /, '')}`);
    }
    const refreshed = await host.toolchain.onlineDevices();
    const target = refreshed.find(device => device.serial === launched.serial);
    if (target === undefined) {
        throw new Error(`android_boot: the AVD "${reference}" booted as ${launched.serial} but adb no longer lists it as `
