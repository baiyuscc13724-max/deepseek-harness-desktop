...
export function detectProject(projectPath) {
    ...
    if (!isDirectory(absolute)) {
        throw new Error(`android_build_run: projectPath is not a directory: ${projectPath} — pass the Android project root `
    ...
    for (;;) {
        ...
        if (settings !== undefined)
            return { ...gradleCommandFor(current), root: current, buildFile: settings };
    ...
    if (build !== undefined)
        return { ...gradleCommandFor(absolute), root: absolute, buildFile: build };
    throw new Error(`android_build_run: ${projectPath} is not a Gradle project — no settings.gradle[.kts] in it or in any `
...
function gradleCommandFor(root) {
    const wrapper = join(root, process.platform === 'win32' ? 'gradlew.bat' : 'gradlew');
    if (isFile(wrapper))
        return { gradleCommand: wrapper, gradleSource: 'wrapper' };
 ...
 */
export function assembleTask(variant, module) {
    const normalized = variant.trim() === '' ? 'debug' : variant.trim();
    const task = `assemble${normalized.charAt(0).toUpperCase()}${normalized.slice(1)}`;
    if (module === undefined || module.trim() === '')
        return task;
    const path = module.trim().replace(/^:+|:+$/g, '');
    return `:${path}:${task}`;
}
/** The argument vector handed to Gradle. Exported for dry-run verification. */
export function assembleGradleArgs(task, extraArgs = []) {
    ...
    return [task, '--console=plain', '--stacktrace', ...extraArgs];
}
/** Run one Gradle invocation, keeping the last OUTPUT_TAIL_LINES lines. */
export async function runGradleBuild(project, args, signal) {
    const child = spawn(project.gradleCommand, [...args], {
        cwd: project.root,
    ...
    try {
        return await new Promise((resolveRun, rejectRun) => {
            child.once('error', error => rejectRun(new Error(`android_build_run: could not run ${project.gradleCommand}: ${errorMessage(error)}`
 ...
 */
export function filterBuildOutput(lines, limit = ERROR_TAIL_LINES) {
...
export function buildFailureDetail(lines) {
 ...
 */
export function findBuiltApk(root, variant = 'debug') {
    const found = [];
    const walk = (directory, depth) => {
        if (depth > APK_SEARCH_MAX_DEPTH)
            return;
        let entries;
        try {
        ...
        for (const entry of entries) {
            ...
            if (!isDirectory(child))
                continue;
            if (entry === 'build') {
                collectApks(join(child, 'outputs', 'apk', variant), found);
                ...
                if (isDirectory(apkRoot)) {
                    for (const flavour of readdirSync(apkRoot)) {
                        collectApks(join(apkRoot, flavour, variant), found);
                    }
                }
                continue;
            }
    ...
    collectApks(join(root, 'build', 'outputs', 'apk', variant), found);
    walk(root, 0);
    if (found.length === 0)
        return undefined;
...
function collectApks(directory, into) {
    ...
    for (const entry of readdirSync(directory)) {
        ...
        try {
            ...
            if (!info.isFile())
                continue;
            if (into.some(existing => existing.path === path))
                continue;
 ...
 */
export function applicationIdFromOutputMetadata(outputDir) {
    ...
    try {
        ...
        if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed))
            return undefined;
        const value = parsed.applicationId;
 ...
 */
export function applicationIdFromSources(moduleDir, root) {
    ...
    for (;;) {
        for (const name of BUILD_FILES) {
            ...
            try {
                ...
                const match = /\bapplicationId\s*(?:=\s*)?["']([A-Za-z][A-Za-z0-9_]*(?:\.[A-Za-z][A-Za-z0-9_]*)+)["']/u.exec(text)
                    ?? /\bnamespace\s*(?:=\s*)?["']([A-Za-z][A-Za-z0-9_]*(?:\.[A-Za-z][A-Za-z0-9_]*)+)["']/u.exec(text);
                if (match !== null)
                    return { packageName: match[1], source: 'build-script' };
            }
            catch {
        ...
        if (isFile(manifest)) {
            try {
                ...
                const match = /<manifest[^>]*\bpackage\s*=\s*"([^"]+)"/u.exec(text);
                if (match !== null)
                    return { packageName: match[1], source: 'manifest' };
            }
            catch {
        ...
        if (current === root)
            break;
        const parent = dirname(current);
        if (parent === current || !parent.startsWith(root + sep) && parent !== root)
            break;
        current = parent;
    }
    return undefined;
 ...
 * back up to `<module>`.
 */
export function moduleDirectoryOf(apkPath) {
    let current = dirname(apkPath);
    for (let depth = 0; depth < 8; depth += 1) {
        const parent = dirname(current);
        if (basename(current) === 'build')
            return parent;
        if (parent === current)
            break;
        current = parent;
...
export function resolveApplicationId(apk, root) {
    const fromMetadata = applicationIdFromOutputMetadata(apk.outputDir);
    if (fromMetadata !== undefined)
        return { packageName: fromMetadata, source: 'output-metadata' };
    const fromSources = applicationIdFromSources(moduleDirectoryOf(apk.path), root);
    if (fromSources !== undefined)
        return fromSources;
    throw new Error(`android_build_run: the build produced ${apk.path} but its applicationId could not be determined — `
 ...
 */
export async function buildRun(options) {
    const { project, toolchain, device, deviceSummary, signal } = options;
    const variant = options.variant === undefined || options.variant.trim() === '' ? 'debug' : options.variant.trim();
    ...
    if (exitCode !== 0) {
        throw new Error(`android_build_run: ${basename(project.gradleCommand)} ${task} failed (exit ${String(exitCode)}) in `
            + `${project.root}:\n${buildFailureDetail(lines)}`);
    }
    const apk = findBuiltApk(project.root, variant);
    if (apk === undefined) {
        throw new Error(`android_build_run: ${task} succeeded but no APK was found under ${project.root}`
    ...
    try {
        ...
        if (failure !== null) {
            throw new Error(`the device refused the package (${failure[1]}) — an INSTALL_FAILED_UPDATE_INCOMPATIBLE or `
 ...
 */
export async function launchPackage(toolchain, serial, packageName) {
    ...
    if (/No activities found|Error:|monkey aborted/u.test(output)) {
        ...
        throw new Error(`could not launch ${packageName} on ${serial}: ${output.trim().split('\n').slice(0, 4).join(' ')} — `
