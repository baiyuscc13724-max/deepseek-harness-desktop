 ...
 * missing.
 */
export const ANDROID_LABEL_HINT = 'Android exposes no app label over adb (an app’s android:label lives in its compiled resources, '
 ...
 */
export function parsePmListPackages(stdout, serial) {
    ...
    if (entries.length === 0) {
        ...
        throw new Error(`could not parse the \`pm list packages\` output${where} — it carried no package lines (got `
 ...
 */
export function parseDumpsysPackageVersions(stdout) {
    ...
    for (const rawLine of stdout.split('\n')) {
        ...
        if (versionName !== null) {
            ...
            if (value !== '' && value !== 'null')
                versions.get(current).version = value;
            continue;
        }
        ...
        if (versionCode !== null) {
            ...
            if (Number.isSafeInteger(value))
                versions.get(current).versionCode = value;
        }
    }
    return versions;
}
 ...
 */
export async function listAndroidApps(toolchain, serial, options = {}) {
    ...
    try {
        ...
        throw new Error(`could not list the packages installed on ${serial}: ${errorMessage(error)} — the device must be `
    ...
    try {
        ...
        throw new Error(`could not list the SYSTEM packages on ${serial}: ${errorMessage(error)} — without that listing the `
...
export function filterAndroidApps(apps, options = {}) {
...
export function noMatchCandidateLines(apps, limit = NO_MATCH_CANDIDATE_LIMIT) {
    return [...apps]
 ...
 */
export function noMatchListingHint(listedTotal, includedSystem) {
 ...
 */
export function resolveAppByName(tool, apps, name, deviceName) {
    ...
    if (pool.length === 0) {
        ...
        throw new Error(`${tool}: no installed package matches "${wanted}" on ${deviceName} — run android_list_apps to see `
    ...
    const ambiguous = (exact.length > 1 ? exact : pool)
        ...
        .map(app => (app.version === undefined ? app.packageName : `${app.packageName} — ${app.version}`));
    throw new Error(`${tool}: ${pool.length} installed packages match "${wanted}" on ${deviceName} — pass packageName (or a `
