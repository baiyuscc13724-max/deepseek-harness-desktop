...
`(\\d[\\d,]*(?:\\.\\d+)?)\\s*([万亿kmwKMW]?)\\s*([^\\d${TERMINATORS}]+)(?!\\d)`, 'gu');
 ...
 */
export function parseCountsFromLabel(label) {
    const counts = [];
...
export function rowCountFor(row, key) {
 ...
 */
export function aggregateRowLabel(node) {
    const parts = [];
    const push = (value) => {
        if (value === undefined)
            return;
 ...
 */
function clusterSiblings(children, minRepeatedRows, screenHeight) {
    ...
    for (const child of children) {
        ...
        if (w <= 0 || h < MIN_ROW_HEIGHT_PX || h > maxHeight)
            continue;
    ...
    for (const bucket of byType.values()) {
        ...
        for (const node of sorted) {
            ...
            const tolerance = Math.max(HEIGHT_TOLERANCE_PX, Math.min(previous.bounds.h, node.bounds.h) * HEIGHT_TOLERANCE_RATIO);
            if (Math.abs(node.bounds.h - previous.bounds.h) <= tolerance)
                current.push(node);
            else {
 ...
 */
export function detectListRows(roots, options) {
    ...
    const visit = (node) => {
        if (node.children.length >= minRepeatedRows) {
            for (const cluster of clusterSiblings(node.children, minRepeatedRows, options.bounds.height)) {
                if (!cluster.some(member => aggregateRowLabel(member) !== undefined))
                    continue;
                const group = nextGroup;
                nextGroup += 1;
                for (const member of cluster)
                    candidates.push({ node: member, group });
            }
        }
    ...
    const outermost = candidates.filter(candidate => !candidates.some(other => other !== candidate && strictlyContains(other.node.bounds, candidate.node.bounds)));
    ...
    for (const candidate of outermost) {
        ...
        if (onscreen.some(kept => sameFrame(kept.node.bounds, candidate.node.bounds)))
            continue;
        onscreen.push(candidate);
    }
    const ordered = [...onscreen].sort((a, b) => a.node.bounds.y - b.node.bounds.y || a.node.bounds.x - b.node.bounds.x);
 ...
 */
export function planRowTap(rows, index, fractionX, fractionY, bounds) {
    requireFraction(fractionX, 'x');
    requireFraction(fractionY, 'y');
    if (!Number.isInteger(index) || index < 0 || index >= rows.length) {
        throw new Error(`android_tap_row: row ${index} does not exist — the current screen has ${rows.length} visible row(s). `
    ...
    if (isOffscreenBounds(row.frame, bounds)) {
        throw new Error(`android_tap_row: row ${index} lies off-screen — scroll it into view first (android_interact with a `
 ...
 */
export function requireCountKey(row, key) {
    ...
    if (value === undefined) {
        ...
        throw new Error(`android_tap_row: cannot verify a ${JSON.stringify(key)} change — ${shown}. `
...
export function sanitizeCountDelta(delta) {
    if (delta !== 1 && delta !== -1) {
 ...
 * cannot be compared). */
export function rowsStayedPut(before, after) {
 ...
 */
export function verifyCountChange(before, after, key, delta) {
