...
function ocrCacheInstallDir(sourceSha256) {
 ...
 */
export function resolveOcrSwiftSource() {
 ...
 */
function validCachedBinary(sourceSha256) {
...
export function execOcr(binary, imagePath, signal, timeoutMs = OCR_EXEC_TIMEOUT_MS) {
 ...
 */
async function compileOcrHelper() {
    const source = resolveOcrSwiftSource();
    const swiftc = resolveSwiftc();
    ...
    try {
        ...
        await new Promise((resolveRun, rejectRun) => {
            execFile(tmp, [], { timeout: 60_000, maxBuffer: 1024 * 1024 }, error => {
                if (error === null || (typeof error.code === 'number' && error.code === 2))
                    resolveRun();
                else
                    rejectRun(new Error(`compiled OCR helper failed its sanity launch: ${error.message}`));
            });
        });
...
export async function ensureOcrBinary() {
    ...
    if (resolved.available || resolved.compilable !== true)
        return resolved;
...
export function parseOcrOutput(stdout) {
    ...
    for (const entry of record.items) {
        ...
        if (text === '' || confidence === undefined || x === undefined || y === undefined || w === undefined || h === undefined)
            continue;
        if (confidence < 0 || confidence > 1 || x < 0 || y < 0 || w < 0 || h < 0)
            continue;
 ...
 */
export function filterOcrItems(items, query, minConfidence = 0) {
    const needle = query !== undefined && query.trim() !== '' ? query.trim().toLowerCase() : undefined;
    return items.filter(item => (needle === undefined || item.text.toLowerCase().includes(needle))
 ...
 */
export function pixelRectToNormalized(rect, pixelSize) {
...
export function normalizedRectToPixels(rect, pixelSize) {
    requireSize(pixelSize, 'pixelSize');
    return {
        x: rect.x * pixelSize.width,
        y: rect.y * pixelSize.height,
...
export function pixelRectToNormalizedCenter(rect, pixelSize) {
    requireSize(pixelSize, 'pixelSize');
