...
import { detectListRows, planRowTap, requireCountKey, sanitizeCountDelta, verifyCountChange, } from './list-rows.js';
import { OCR_FALLBACK_HINT, ScreenshotStore, boundsSchema, captureScreenshot, deviceSchema, deviceSummaryOf, errorMessage, pointSchema, renderJson, round4, screenshotMeta, sizeSchema, sleep, } from './tool-uitree.js';
import { hasLabeledNode, readUiTree, screenBoundsOf, } from './uitree.js';
import { IMAGE_REF_SCHEMA, renderJsonWithImage } from './vision.js';
/** Registered list-row tool names, in registration order. */
export const ANDROID_ROW_TOOL_NAMES = ['android_ui_rows', 'android_tap_row'];
...
function capRows(rows) {
    ...
    if (bytes(rows) <= ROWS_CAP_BYTES)
        return { rows, truncated: false };
    const kept = [...rows];
    while (bytes(kept) > ROWS_CAP_BYTES && kept.length > 1)
        kept.pop();
 ...
 */
function noRowsHint(roots, omittedOffscreen) {
...
export function createAndroidRowTools(host, options = {}) {
    ...
    const readRows = async (tool, serial) => {
        let roots;
        try {
    ...
    const androidUiRows = defineTool({
        ...
        async execute(args) {
            ...
            if (sample.rows.length === 0)
                hints.push(noRowsHint(sample.roots, sample.omittedOffscreen));
            return {
    ...
    const androidTapRow = defineTool({
        ...
        async execute(args, exec) {
            ...
            if (expectation !== undefined)
                requireCountKey(plan.row, expectation.key);
            if (sample.screen.width <= 0 || sample.screen.height <= 0) {
                throw new Error('android_tap_row: the dump reported a zero-size display, so a tap cannot be placed — re-run '
            ...
            const screenshot = await captureScreenshot('android_tap_row', screenshots, host, device, vision === undefined ? undefined : { services: vision, exec });
            return {
                action: 'tap-row',
