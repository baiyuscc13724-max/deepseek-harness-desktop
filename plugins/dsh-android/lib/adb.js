...
export class AdbError extends Error {
    ...
    stderr;
    exitCode;
    constructor(message, args, stderr = '', exitCode = null) {
        super(message);
        this.args = args;
        this.stderr = stderr;
...
export function resolveAdbBinary() {
    ...
    if (env !== undefined && env !== '') {
        if (isExecutableFile(env))
            return { available: true, source: 'env', command: env };
    ...
    if (onPath !== undefined)
        return { available: true, source: 'path', command: onPath };
    for (const root of sdkRootCandidates()) {
        const candidate = join(root, 'platform-tools', executableName('adb'));
        if (isExecutableFile(candidate))
            return { available: true, source: 'sdk', command: candidate };
    }
    return {
        available: false,
        source: 'unavailable',
 ...
 */
export function resolveEmulatorBinary(adb) {
    ...
    if (onPath !== undefined)
        return { available: true, command: onPath };
    return {
        available: false,
...
function describeExecFailure(error, timeoutMs) {
    ...
    if (error.code === 'ENOENT')
        return 'the executable was not found';
    return `exit ${String(error.code)}`;
}
function execBinary(command, args, label, options) {
    ...
    return new Promise((resolve, reject) => {
        execFile(command, [...args], {
            ...
            if (error !== null) {
                ...
                reject(new AdbError(`${label} ${args.join(' ')} failed (${cause})${detail === '' ? '' : `: ${detail}`}`, args, stderr, exitCode));
                return;
            }
...
export class AdbToolchain {
     ...
     */
    execOut(serial, command, options = {}) {
        ...
        return new Promise((resolve, reject) => {
            execFile(adb, ['-s', serial, 'exec-out', ...command], {
                ...
                if (error !== null) {
                    ...
                    reject(new AdbError(`adb exec-out ${command.join(' ')} failed (${cause})${detail === '' ? '' : `: ${detail}`}`, command, stderr.toString('utf8'), typeof error.code === 'number' ? error.code : null));
                    return;
                }
                resolve(stdout);
            });
    ...
    async listDevices() {
        ...
        for (const line of stdout.split('\n')) {
            ...
            const state = ['device', 'offline', 'unauthorized', 'recovery', 'sideload'].find(known => known === rawState) ?? 'unknown';
            const fields = new Map();
            for (const pair of (rest ?? '').trim().split(/\s+/)) {
                const colon = pair.indexOf(':');
                if (colon > 0)
                    fields.set(pair.slice(0, colon), pair.slice(colon + 1));
            }
    ...
    async deviceDetails(device) {
        const details = { serial: device.serial };
        try {
            ...
            if (release !== undefined && release !== '')
                details.androidVersion = release;
            const sdkNumber = Number(sdk);
            if (Number.isSafeInteger(sdkNumber) && sdkNumber > 0)
                details.sdk = sdkNumber;
        }
        catch {
    ...
    async screenSize(serial) {
        ...
        if (size === undefined)
            throw new AdbError(`dsh-android: \`wm size\` returned no size for ${serial}: ${output.trim()}`, ['wm', 'size']);
        return size;
    }
    /** Poll `sys.boot_completed` until the device finishes booting. */
    async waitForBoot(serial, timeoutMs = DEFAULT_BOOT_TIMEOUT_MS) {
...
function execEmulator(command, args, options = {}) {
 ...
 */
export async function bootAvd(toolchain, avdName, options = {}) {
    ...
    while (serial === undefined) {
        ...
        const emulators = (await toolchain.listDevices().catch(() => [])).filter(device => device.emulator);
