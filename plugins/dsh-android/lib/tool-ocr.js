...
import { filterOcrItems, pixelRectToNormalizedCenter, rectCenter, } from './ocr-backend.js';
import { OCR_POLL_INTERVAL_MS, ScreenshotStore, TAP_SETTLE_MS, boundsSchema, captureScreenshot, deviceSchema, deviceSummaryOf, errorMessage, expectedSchema, losslessNumber, pointSchema, readOcrOnce, renderJson, requirePixelSize, round2, round4, runOcr, runTapExpectation, screenshotMeta, sizeSchema, sleep, pollForText, tapExpectation, } from './tool-uitree.js';
import { IMAGE_REF_SCHEMA, renderJsonWithImage } from './vision.js';
/** Registered Vision-OCR tool names, in registration order. */
export const ANDROID_OCR_TOOL_NAMES = ['android_find_text', 'android_tap_text', 'android_wait_for'];
...
export function sanitizeMinConfidence(value) {
    if (value === undefined)
        return OCR_DEFAULT_MIN_CONFIDENCE;
 ...
 * rect, capped at 8).
 */
export function resolveOcrTextTarget(items, query,
/** Every OCR item BEFORE the confidence filter, for the near-miss report. */
unfiltered = items, minConfidence = 0) {
    const matches = (pool, mode) => mode === 'exact'
    ...
    if (pool.length === 0) {
        ...
        if (nearMiss !== undefined) {
            throw new Error(`android_tap_text: ${JSON.stringify(nearMiss.text)} IS on the current screen, but its OCR `
        ...
        throw new Error(`android_tap_text: no recognized text matches ${JSON.stringify(query)} on the current screen — `
    ...
    const unique = pool.filter((item, index) => !pool.slice(0, index).some(other => other.text === item.text
    ...
    if (unique.length > 1) {
        ...
        throw new Error(`android_tap_text: ${unique.length} OCR matches for ${JSON.stringify(query)} — use a more specific query, `
            + 'or raise min_confidence to drop weak matches. Candidates:\n'
            + shown.map((item, index) => `  ${index + 1}) text=${JSON.stringify(item.text)} confidence=${round2(item.confidence)} `
...
function capOcrItems(items) {
    const bytes = (list) => Buffer.byteLength(JSON.stringify(list), 'utf8');
    if (bytes(items) <= OCR_CAP_BYTES)
        return { items, truncated: false };
    const kept = [...items];
    while (bytes(kept) > OCR_CAP_BYTES && kept.length > 1)
        kept.pop();
...
export function createAndroidOcrTools(host, options = {}) {
    ...
    const androidWaitFor = defineTool({
        ...
        async execute(args, exec) {
            ...
            const timeoutMs = Math.min(WAIT_MAX_TIMEOUT_MS, Math.max(0, typeof args.timeout_ms === 'number' && Number.isFinite(args.timeout_ms) ? args.timeout_ms : WAIT_DEFAULT_TIMEOUT_MS));
            const minConfidence = sanitizeMinConfidence(args.min_confidence);
            const device = await host.resolveTarget(args.serial);
            ...
            }, text, mode, timeoutMs, OCR_POLL_INTERVAL_MS, minConfidence, exec.signal);
            return {
    ...
    const androidTapText = defineTool({
        ...
        async execute(args, exec) {
            ...
            const screenshot = await captureScreenshot('android_tap_text', screenshots, host, device, vision === undefined ? undefined : { services: vision, exec });
