 ...
 */
import { ANDROID_CARD_TOOLS, parseAndroidMeta, } from './protocol.js';
...
function hydrateStreamMeta(value) {
    ...
    if (!isRecord(value) || value.streaming !== true)
        return null;
    if (value.state !== 'streaming' && value.state !== 'booted')
        return null;
...
function hydrateScreenshotMeta(value, interact) {
    ...
    if (interact) {
        ...
        if (typeof action !== 'string' || !INTERACT_ACTIONS.has(action))
            return null;
    }
    const path = isPosixAbsolutePath(value.path) ? value.path : undefined;
    if (path === undefined || !isByteCount(value.bytes))
        return null;
    const device = parseDevice(value.device);
    if (device === undefined)
        return null;
...
function hydrateBuildRunMeta(value) {
    if (!isRecord(value) || value.state !== 'launched')
        return null;
...
function hydrateAndroidMetaValue(toolName, value) {
    if (toolName === ANDROID_CARD_TOOLS.boot)
        return hydrateStreamMeta(value);
    ...
    if (toolName === ANDROID_CARD_TOOLS.buildRun)
        return hydrateBuildRunMeta(value);
    return null;
}
/**
 ...
 */
export function hydrateAndroidMeta(toolName, block) {
 ...
 */
export function resolveAndroidMeta(toolName, block) {
    if (!('kind' in block) || block.kind !== 'tool-result' || block.isError)
        return undefined;
    const projected = parseAndroidMeta(block.meta);
    if (projected !== undefined)
        return { meta: projected, source: 'meta' };
