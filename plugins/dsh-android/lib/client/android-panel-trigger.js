...
export function registerAndroidPanelSource(source) {
...
export function subscribeAndroidPanelSources(listener) {
    sourceListeners.add(listener);
    return () => { sourceListeners.delete(listener); };
}
/** The registry change counter (stable between changes, for `useSyncExternalStore`). */
export function androidPanelSourcesVersion() {
 ...
 */
export function androidPanelSourcesSnapshot() {
 ...
 */
export function hasAndroidPanelSourceForSession(sessionId) {
    if (sessionId === '')
        return false;
...
export function resolveAndroidPanelSource(callId) {
    return sources.get(callId);
}
/** Register a card's settled result while it is mounted. */
export function useAndroidPanelSource(enabled, source) {
    useEffect(() => {
 ...
 */
export function installAndroidPanelRowTrigger(doc, open) {
    const onClick = (event) => {
        const target = event.target;
        if (!(target instanceof Element))
            return;
        if (androidPanelClickIsInteractive(target))
            return;
