...
import { ANDROID_DRAG_MOVE_SAMPLE_MS, androidGestureActionOf, androidRouteErrorTextOf, normalizePointerPoint, postAndroidControl, requestStreamGrant, } from './protocol.js';
/**
 ...
 */
export function useAndroidStream(options) {
    ...
    const reportLive = useCallback((next) => {
        setLive(next);
    ...
    const autoReGrant = useCallback(() => {
        ...
        if (switchSettleRef.current !== undefined)
            return;
    ...
    const onFrameLoad = useCallback((naturalWidth) => {
        if (!(naturalWidth > 0))
            return;
    ...
    useEffect(() => {
        ...
        void requestStreamGrant(fetcher ?? fetch, {
            device: serial === undefined ? {} : { serial },
        ...
        return cleanup;
    }, [attempt, serial, sessionId, enabled, fetcher, reportLive, seededGrant, startSettleTimer, clearSettleTimer]);
    ...
    const control = useCallback((action) => {
        if (serial === undefined || serial === '')
            return;
        void postAndroidControl(fetcher ?? fetch, serial, action, sessionId).then(result => {
            if (result.ok && result.result.rotation !== undefined)
                setRotation(result.result.rotation);
        });
    }, [serial, sessionId, fetcher]);
    const sendButton = useCallback((name) => {
    ...
    const onPointerMove = (event) => {
        ...
        if (active === undefined || active.id !== event.pointerId)
            return;
        ...
        const now = Date.now();
        if (now - active.sampledAt < ANDROID_DRAG_MOVE_SAMPLE_MS)
            return;
