...
export const ANDROID_PANEL_SIZE_MODE_FIT = { kind: 'fit' };
/** The zoom percentages offered by the percent mode. */
export const ANDROID_PANEL_PERCENT_OPTIONS = [50, 75, 100, 125];
/**
 ...
 */
export const ANDROID_PANEL_PRESET_OPTIONS = [
...
export function androidPanelFrameLayoutOf(naturalWidth, naturalHeight) {
    ...
    const displayH = positiveOr(naturalHeight, Math.round(displayW * ANDROID_PANEL_FALLBACK_LOGICAL_HEIGHT / ANDROID_PANEL_FALLBACK_LOGICAL_WIDTH));
 ...
 */
export function androidDeviceScaleOf(naturalWidth, naturalHeight) {
    if (naturalWidth === undefined && naturalHeight === undefined)
        return ANDROID_PANEL_DEVICE_SCALE_FALLBACK;
 ...
 */
export function androidPanelDisplayLogicalWidthOf(naturalWidth, naturalHeight) {
    const layout = androidPanelFrameLayoutOf(naturalWidth, naturalHeight);
 ...
 */
export function androidPanelSnapPxOf(value) {
 ...
 */
export function androidPanelFrameWidthOf(mode, naturalWidth, naturalHeight) {
    switch (mode.kind) {
        case 'fit': return '100%';
        case 'percent': {
...
export function androidPanelSizeModeIdOf(mode) {
    switch (mode.kind) {
        case 'fit': return 'fit';
...
export function androidPanelSizeModeOf(id) {
 ...
 */
export const ANDROID_PANEL_QUICK_SIZE_OPTIONS = ['fit', 'percent-100', 'preset-S', 'preset-M'].map(id => {
    const option = ANDROID_PANEL_SIZE_OPTIONS.find(candidate => candidate.id === id);
    if (option === undefined)
        throw new RangeError(`dsh-android: quick size ${id} is missing from the dropdown roster`);
