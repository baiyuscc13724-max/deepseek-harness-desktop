...
export function androidResultSummaryOf(block) {
    ...
    for (const item of block.content) {
        ...
        if (bytes === undefined && width === undefined && height === undefined && action === undefined && path === undefined)
            continue;
        if (bytes !== undefined)
            summary.bytes = bytes;
        if (width !== undefined)
...
export function androidResultTextOf(block) {
    if (!('kind' in block))
        return null;
    const parts = [];
    for (const item of block.content) {
