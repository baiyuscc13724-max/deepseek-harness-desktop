...
export const ANDROID_PANEL_AUTO_OPEN_TOOLS = [
...
const autoOpenCalls = new Map();
/** The one-shot registry key: session + call identity, like openpencil. */
export function androidPanelAutoOpenKey(sessionId, callId) {
...
export function rememberAndroidPanelAutoOpenCall(key) {
    autoOpenCalls.delete(key);
...
export function takeAndroidPanelAutoOpenCall(key) {
    pruneAutoOpenCalls();
    if (!autoOpenCalls.has(key))
        return false;
    autoOpenCalls.delete(key);
    return true;
}
/** Forget the key after an error so a later successful result may take it. */
export function forgetAndroidPanelAutoOpenCall(key) {
    autoOpenCalls.delete(key);
}
/**
 ...
 */
export function androidPanelAutoOpenShouldOpen(input) {
    if (input.isError)
        return false;
    if (!ANDROID_PANEL_AUTO_OPEN_TOOLS.includes(input.toolName))
        return false;
    if (input.sessionId === '' || input.sessionId !== input.currentSessionId)
        return false;
    if (typeof input.blockTime !== 'number' || !Number.isFinite(input.blockTime))
        return false;
    if (input.blockTime < input.activatedAt)
        return false;
    return true;
}
