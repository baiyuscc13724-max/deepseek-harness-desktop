        ...
        project ? h(React.Fragment, null,
          ...
          h("details", null, h("summary", null, t("projectAdvanced")),
            ...
            h("div", { className: "dat-project-form" },
              ...
              ) : h("div", { className: "dat-note dat-project-span" }, t("projectPairingReady")),
              project.role === "owner" ? h("label", { className: "dat-project-span" }, h("span", { className: "dat-label" }, t("projectRelayUrl")), h("input", { className: "dat-field", inputMode: "url", value: relayUrl, maxLength: 2048, placeholder: t("projectRelayPlaceholder"), onChange: function (event) { setRelayUrl(event.target.value); } })) : null,
              h("div", { className: "dat-actions dat-project-span" }, project.role === "owner" ? h(Button, { small: true, disabled: !!busy || !relayUrl.trim(), onClick: function () { run("set-relay", { relayUrl: relayUrl.trim() }); } }, t("projectSaveRelay")) : null, relay.connected ? h(Button, { small: true, disabled: !!busy, onClick: function () { run("disconnect-remote", {}); } }, t("projectDisconnectRemote")) : h(Button, { small: true, disabled: !!busy || !relay.enabled || !relay.channelReady, onClick: function () { run("connect-remote", { role: relay.role || (project.role === "owner" ? "authority" : "collaborator") }); } }, t("projectConnectRemote")))
            ),
