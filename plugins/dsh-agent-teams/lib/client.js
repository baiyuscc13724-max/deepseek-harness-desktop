window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var zh = {
      ...
      pending: "待处理", in_progress: "进行中", completed: "已完成", blocked: "受阻", ready: "等待任务", running: "工作中", idle: "本轮工作已完成", provisioning: "正在启动", shutting_down: "正在停止", closing: "正在关闭", retired: "已结束协作", failed: "失败", delivered: "已送达", closedStatus: "已关闭",
      assignee: "执行成员", unassigned: "未分配", blockedBy: "正在等待：{value}", dependencySources: "还需其他团队完成：{value}", conflicts: "文件范围可能冲突：{value}", files: "文件：{value}", filesHidden: "为保护工作区信息，此页面不显示文件路径；需要时请让负责人核对。", taskFallback: "任务 {id}", lastActivity: "最后活动：{value}", deliveryEvent: "{from} → {to} · {status}", crossDelivery: "{fromTeam} → {toTeam} · {from} → {to} · {status}", taskDetail: "任务详情", taskDetailUnavailable: "此任务已结束或当前无法读取。请返回任务板选择其他任务，或查看任务历史。", taskSelectionExpired: "任务信息刚刚更新，原详情已关闭。请从当前任务列表重新选择。", blockedTaskReason: "受阻原因", blockedTaskUnknown: "暂未提供具体阻塞原因。", blockedTaskNext: "下一步：等待列出的依赖完成；如果信息已过期，请让负责人刷新依赖或重新协调任务。", taskDependencies: "依赖任务", taskEvents: "相关实时事件", taskRef: "任务编号", taskBackToBoard: "返回任务板", taskWorkflow: "实时工作流", taskWorkflowHint: "状态和动态会随团队实时更新；没有可靠记录的环节不会猜测。", taskOverview: "任务概览", taskLiveEvents: "实时动态", taskLiveEventsHint: "这里只显示与当前任务有关的最新协作记录。", taskLiveConnected: "详情会随团队状态自动更新。", taskBlockedBranch: "当前阻塞", taskBlockedClear: "当前没有阻塞", taskBlockedBranchHint: "阻塞不是必经阶段；只有任务实时报告受阻时才会点亮。", taskStageCurrent: "当前阶段", taskStageReached: "已有记录", taskStageUpcoming: "尚未到达", taskStageUnknown: "暂无可靠记录", taskCreatedAt: "进入待处理", taskStartedAt: "开始执行", taskCompletedAt: "完成时间", taskNextStep: "当前下一步", taskNextPending: "等待负责人或成员认领；存在依赖时先完成依赖。", taskNextProgress: "执行成员正在推进；下方动态会持续更新。", taskNextCompleted: "任务已经完成，可在实时动态中回看相关协作。", taskTimelineLimited: "仅显示最新 {count} 条相关动态。",
    ...
    function safeTaskDetail(task) {
      ...
      var blockedBy = relationIds(task.blockedBy), dependencies = relationIds(task.dependencies).concat(relationIds(task.dependsOn));
      var reason = typeof task.blockReason === "string" ? task.blockReason.trim() : typeof task.blockedReason === "string" ? task.blockedReason.trim() : "";
      return { task: task, stateKind: taskStateKind(task), filesText: arrayText(task.files || task.fileScope), blockedBy: blockedBy, conflicts: relationIds(task.conflictsWith), dependencies: dependencies, reason: reason };
    }
