...
class ProjectBusinessSyncRuntime {
  ...
  constructor({ projectEntry, clock = Date.now, scheduler = setTimeout, queueMicrotaskImpl = queueMicrotask, randomBytesImpl = randomBytes, createHmacImpl = createHmac, taskDelegate, automationDelegate, wakeScheduler = () => undefined, pumpLimit = DEFAULT_PUMP_LIMIT, refreshMs = DEFAULT_REFRESH_MS, retryMaxMs = DEFAULT_RETRY_MAX_MS } = {}) {
    ...
    if (typeof clock !== "function" || typeof scheduler !== "function" || typeof queueMicrotaskImpl !== "function" || typeof randomBytesImpl !== "function" || typeof createHmacImpl !== "function" || typeof wakeScheduler !== "function") throw new TypeError("runtime clocks and schedulers must be functions");
    this.projectEntry = projectEntry; this.clock = clock; this.scheduler = scheduler; this.queueMicrotaskImpl = queueMicrotaskImpl; this.randomBytesImpl = randomBytesImpl; this.createHmacImpl = createHmacImpl; this.taskDelegate = taskDelegate; this.automationDelegate = automationDelegate; this.wakeScheduler = wakeScheduler;
  ...
  async #bindAuthority(syncContext, refFactory, syncRefKey) {
    ...
    try {
      ...
      const taskService = new ProjectTaskCommandService({ store: taskStore, actorResolver, now: this.clock, wakeScheduler: this.wakeScheduler });
      ...
      const systemTaskService = new ProjectTaskCommandService({ store: taskStore, actorResolver: (execution, projectRef) => { if (execution !== systemExecution) throw runtimeError("automation runner execution is invalid", "PROJECT_TASK_ACTOR_UNRESOLVED"); return { projectRef, actorRef: systemActorRef, kind: "system", authorities: [] }; }, now: this.clock, wakeScheduler: this.wakeScheduler });
