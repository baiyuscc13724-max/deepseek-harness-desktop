...
final class MobileUiAdapter {
    ...
    private static final String FILE_ENTRY_JS = "(" +
        ...
        "var waitForSessionReady=function(timeoutMs){return new Promise(function(resolve){if(window.__harnessMobileCurrentSessionId){resolve(true);return;}var settled=false;var timer=0;var finish=function(ok){if(settled)return;settled=true;window.removeEventListener('harness-mobile-session-history-receipt',onReceipt);clearTimeout(timer);resolve(ok);};var onReceipt=function(event){var sessionId=event&&event.detail&&event.detail.sessionId;if(sessionId&&sessionId===window.__harnessMobileCurrentSessionId)finish(true);};window.addEventListener('harness-mobile-session-history-receipt',onReceipt);timer=setTimeout(function(){finish(false);},timeoutMs);if(window.__harnessMobileCurrentSessionId)finish(true);});};" +
        ...
        "var dispatchImages=function(files){if(!files||!files.length)return Promise.resolve(false);var deliver=function(){var before=railCounts(files);setAttachmentState('pending',files.length,'正在添加图片…');try{dispatchDrop(files);}catch(dropError){console.warn('Harness Mobile official image drop intake failed',dropError);setAttachmentState('error',files.length,'无法添加图片，请重试');return Promise.resolve(false);}return waitForRail(files,before,8000).then(function(accepted){if(accepted){setAttachmentState('success',files.length,'已添加 '+files.length+' 张图片');return true;}console.error('Harness Mobile attachment rail did not accept selected images');setAttachmentState('error',files.length,'图片未进入附件栏，请重试');return false;});};var textarea=currentTextarea();if(workspaceTrigger(textarea)){setAttachmentState('pending',files.length,'请先选择项目，选择后自动添加图片');var card=textarea.closest('[data-composer-card]');if(card)card.click();return waitForImageReady(60000).then(function(ready){if(ready)return deliver();setAttachmentState('error',files.length,'尚未选择项目，图片未添加');return false;});}return deliver();};" +
        "var dispatchDocuments=function(files){var deliver=function(){if(typeof window.__harnessMobileReceiveDocuments==='function')return Promise.resolve(window.__harnessMobileReceiveDocuments(files,setAttachmentState));setAttachmentState('error',files.length,'当前电脑端还不能接收文档');return Promise.resolve(false);};var textarea=currentTextarea();if(!window.__harnessMobileCurrentSessionId&&workspaceTrigger(textarea)){setAttachmentState('pending',files.length,'请先选择项目，选择后自动上传文件');var sessionReady=waitForSessionReady(60000);var card=textarea.closest('[data-composer-card]');if(card)card.click();return Promise.all([waitForImageReady(60000),sessionReady]).then(function(state){if(state[0]&&state[1])return deliver();setAttachmentState('error',files.length,'尚未创建会话，文件未上传');return false;});}return deliver();};" +
        "var dispatchFiles=function(files){var images=[];var documents=[];files.forEach(function(file){if(/^image\\//i.test(file.type||''))images.push(file);else documents.push(file);});var pending=[];if(images.length)pending.push(dispatchImages(images));if(documents.length)pending.push(dispatchDocuments(documents));return Promise.all(pending);};" +
        ...
        "var mount=function(){var card=document.querySelector('[data-composer-card]');if(!card||document.getElementById('harness-mobile-input-button'))return;" +
