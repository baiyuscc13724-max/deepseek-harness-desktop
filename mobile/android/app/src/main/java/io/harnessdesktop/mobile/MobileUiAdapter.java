...
final class MobileUiAdapter {
    ...
    private static final String FILE_ENTRY_JS = "(" +
        ...
        "var currentCard=function(){return document.querySelector('[data-phase=\"active\"] [data-composer-card]')||document.querySelector('[data-harness-mobile-conversation=\"true\"] [data-composer-card]')||document.querySelector('[data-composer-card]');};" +
        "var currentTextarea=function(){var card=currentCard();return card?card.querySelector('[data-composer-input][data-phase],textarea[data-phase]'):null;};" +
        "var legacyTextarea=function(input){return !!(input&&String(input.tagName||'').toLowerCase()==='textarea');};" +
        "var workspaceTrigger=function(input){return !!(input&&input.getAttribute('data-phase')==='inert'&&input.getAttribute('aria-haspopup')==='menu');};" +
        "var unavailable=function(){var input=currentTextarea();if(!input||input.disabled||input.getAttribute('aria-disabled')==='true')return true;if(workspaceTrigger(input))return false;return legacyTextarea(input)?!!input.readOnly:input.getAttribute('contenteditable')!=='true';};" +
        ...
        "var railCounts=function(files){var wanted={};files.forEach(function(file){wanted[file.name]=(wanted[file.name]||0)+1;});var counts={};var card=currentCard();if(!card)return counts;Array.prototype.forEach.call(card.querySelectorAll('[role=\"group\"] img[alt]'),function(image){var name=image.getAttribute('alt')||'';if(wanted[name])counts[name]=(counts[name]||0)+1;});return counts;};" +
        "var waitForRail=function(files,before,timeoutMs){return new Promise(function(resolve){var wanted={};files.forEach(function(file){wanted[file.name]=(wanted[file.name]||0)+1;});var settled=false;var observer=null;var timer=0;var finish=function(ok){if(settled)return;settled=true;if(observer)observer.disconnect();clearTimeout(timer);resolve(ok);};var check=function(){if(!currentCard())return;var after=railCounts(files);var names=Object.keys(wanted);if(names.length&&names.every(function(name){return (after[name]||0)>=(before[name]||0)+wanted[name];}))finish(true);};observer=new MutationObserver(check);observer.observe(document.documentElement,{childList:true,subtree:true,attributes:true,attributeFilter:['alt','src','data-phase','data-harness-mobile-conversation']});timer=setTimeout(function(){finish(false);},timeoutMs);check();});};" +
        "var waitForImageReady=function(timeoutMs){return new Promise(function(resolve){var settled=false;var observer=null;var timer=0;var finish=function(ok){if(settled)return;settled=true;if(observer)observer.disconnect();clearTimeout(timer);resolve(ok);};var check=function(){var input=currentTextarea();if(input&&!unavailable()&&input.getAttribute('data-phase')!=='inert')finish(true);};observer=new MutationObserver(check);observer.observe(document.documentElement,{childList:true,subtree:true,attributes:true,attributeFilter:['disabled','readonly','aria-disabled','contenteditable','data-phase']});timer=setTimeout(function(){finish(false);},timeoutMs);check();});};" +
        ...
        "window.__harnessMobileReceiveSpeech=function(text){if(typeof text!=='string'||!text)return;var input=currentTextarea();if(!input||input.getAttribute('data-phase')==='inert'||unavailable())return;input.focus();if(legacyTextarea(input)){var start=typeof input.selectionStart==='number'?input.selectionStart:input.value.length;var end=typeof input.selectionEnd==='number'?input.selectionEnd:start;var next=input.value.slice(0,start)+text+input.value.slice(end);var setter=Object.getOwnPropertyDescriptor(HTMLTextAreaElement.prototype,'value').set;setter.call(input,next);var caret=start+text.length;input.setSelectionRange(caret,caret);input.dispatchEvent(new Event('input',{bubbles:true}));return;}var selection=typeof window.getSelection==='function'?window.getSelection():null;if(selection&&(!selection.anchorNode||!input.contains(selection.anchorNode))&&document.createRange){var range=document.createRange();range.selectNodeContents(input);range.collapse(false);selection.removeAllRanges();selection.addRange(range);}var inserted=false;try{inserted=typeof document.execCommand==='function'&&document.execCommand('insertText',false,text);}catch(error){}if(!inserted&&typeof InputEvent==='function')input.dispatchEvent(new InputEvent('beforeinput',{bubbles:true,cancelable:true,inputType:'insertText',data:text}));};" +
        ...
        "var makeInput=function(id,accept){var existing=document.getElementById(id);if(existing&&existing.getAttribute('data-harness-mobile-picker')==='true'){existing.accept=accept;existing.multiple=true;return existing;}if(existing&&existing.parentElement)existing.parentElement.removeChild(existing);var input=document.createElement('input');input.type='file';input.accept=accept;input.multiple=true;input.id=id;input.setAttribute('data-harness-mobile-picker','true');if(id==='harness-mobile-photo-input')input.setAttribute('data-harness-mobile-add-photo','true');input.setAttribute('aria-hidden','true');input.tabIndex=-1;input.style.cssText='position:fixed;width:1px;height:1px;opacity:0;pointer-events:none;';input.addEventListener('change',function(){intake(input);});document.body.appendChild(input);return input;};" +
        "var mount=function(){var card=currentCard();var existing=document.getElementById('harness-mobile-input-button');if(!card)return;if(existing&&card.contains(existing)){syncState(existing);return;}var staleEntry=document.getElementById('harness-mobile-input-entry');if(staleEntry&&staleEntry.parentElement)staleEntry.parentElement.removeChild(staleEntry);var staleMenu=document.getElementById('harness-mobile-input-menu');if(staleMenu&&staleMenu.parentElement)staleMenu.parentElement.removeChild(staleMenu);" +
        ...
        "mount();" +
        "var syncOrMount=function(){var card=currentCard();var button=document.getElementById('harness-mobile-input-button');if(button&&card&&card.contains(button)){syncState(button);return;}mount();};" +
        "var affectsEntry=function(records){return records.some(function(record){if(record.type==='attributes')return !!record.target.matches&&record.target.matches('[data-composer-input][data-phase],textarea[data-phase]');var target=record.target&&record.target.nodeType===1?record.target:record.target&&record.target.parentElement;if(target&&target.closest&&target.closest('[data-composer-card]'))return true;var nodes=[].slice.call(record.addedNodes||[]).concat([].slice.call(record.removedNodes||[]));return nodes.some(function(node){return node.nodeType===1&&((node.matches&&node.matches('[data-composer-card]'))||(node.querySelector&&node.querySelector('[data-composer-card]')));});});};" +
        "window.__harnessMobileInputEntryObserver=new MutationObserver(function(records){if(affectsEntry(records))syncOrMount();});" +
        "window.__harnessMobileInputEntryObserver.observe(document.documentElement,{childList:true,subtree:true,attributes:true,attributeFilter:['disabled','readonly','aria-disabled','contenteditable','data-phase']});" +
        "}())";
