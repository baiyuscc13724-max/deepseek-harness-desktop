...
final class MobileUiAdapter {
    ...
    private static final String FILE_ENTRY_JS = "(" +
        ...
        "var fileTransfer=function(files){var transfer=new DataTransfer();for(var i=0;i<files.length;i++)transfer.items.add(files[i]);return transfer;};" +
        "var clipboardPayload=function(files){return {items:files.map(function(file){return {kind:'file',type:file.type,getAsFile:function(){return file;}};}),files:files,types:['Files'],getData:function(){return '';}};};" +
        "var dispatchFiles=function(files){try{var textarea=currentTextarea();if(textarea&&!unavailable()){try{var paste=new Event('paste',{bubbles:true,cancelable:true});Object.defineProperty(paste,'clipboardData',{configurable:true,value:clipboardPayload(files)});textarea.dispatchEvent(paste);return;}catch(pasteError){console.warn('Harness Mobile paste intake failed',pasteError);}}var target=(textarea&&textarea.closest('[data-composer-card]'))||document;var drop=new Event('drop',{bubbles:true,cancelable:true});Object.defineProperty(drop,'dataTransfer',{configurable:true,value:fileTransfer(files)});target.dispatchEvent(drop);}catch(error){console.warn('Harness Mobile attachment intake failed',error);}};" +
