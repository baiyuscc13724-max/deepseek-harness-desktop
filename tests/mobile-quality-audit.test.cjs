...
test('MQA-03：语义刷新覆盖加载时序、无/有 Dialog 状态与关闭后的焦点返回', async () => {
  ...
  assert.match(focus, /const current = topMobileDialog\(dialogs\)/, 'only the top visible layer owns focus')
  assert.match(runtime, /const mobileDialogTriggers = new Map\(\)/, 'each layer retains its own trigger')
  assert.match(focus, /mobileDialogTriggers\.delete\(dialog\)/, 'closed layers release their trigger entries')
  assert.match(focus, /if \(trigger\?\.isConnected && mobileDialogVisible\(trigger\)\) restore = trigger/, 'hidden or detached triggers cannot receive focus')
  assert.match(focus, /if \(!topMobileDialog\(\) && restore\.isConnected && mobileDialogVisible\(restore\)\) restore\.focus\?\.\(\{ preventScroll: true \}\)/, 'deferred restoration must not steal focus from a newly opened modal')
  assert.match(focus, /if \(activeMobileDialog === current\) return/)
  assert.match(focus, /if \(current === topMobileDialog\(\) && mobileDialogVisible\(target\)\) target\.focus\?\.\(\{ preventScroll: true \}\)/, 'stale scheduled focus cannot target a covered or hidden layer')
  assert.match(focus, /event\.defaultPrevented \|\| current !== topMobileDialog\(\)/, 'lower layers must not trap the same Tab event')
