...
test('sign workflow plans up to five exact runs before one protected batch approval and never runs PR code', async () => {
  const source = await load('.github/workflows/pr-preview-sign.yml')
  const workflow = YAML.parse(source)
  assert.ok(workflow.on.workflow_dispatch)
  assert.deepEqual(Object.keys(workflow.on), ['workflow_dispatch'])
  assert.deepEqual(Object.keys(workflow.on.workflow_dispatch.inputs), ['build_run_ids'])
  assert.equal(workflow.on.workflow_dispatch.inputs.build_run_ids.required, true)
  assert.equal(workflow.concurrency.group, 'pr-preview-sign-channel')
  assert.equal(workflow.concurrency['cancel-in-progress'], false)
  assert.equal(workflow.jobs['verify-sign-publish'].needs, 'plan-batch')
  ...
  assert.match(source, /gh run download "\$build_run_id"/)
  ...
  assert.doesNotMatch(source, /matrix:|gh workflow run|workflow_run:/)
  assert.doesNotMatch(source, /HARNESS_COMPONENT_SIGNING_PRIVATE_KEY_BASE64/)
  assert.doesNotMatch(source, /--clobber/)
  assert.doesNotMatch(source, /node .*unsigned|bash .*unsigned|chmod .*unsigned/)
})

test('batch signing preserves one exact four-file recovery artifact per candidate and cannot promote', async () => {
  const [source, signer] = await Promise.all([
    load('.github/workflows/pr-preview-sign.yml'),
    load('scripts/pr-preview-sign.mjs')
  ])
  const workflow = YAML.parse(source)
  assert.equal(workflow['run-name'], 'Plan and sign immutable PR preview batch · workflow run #${{ github.run_number }}')
  assert.match(source, /immutable-candidate-files\.txt/)
  assert.match(source, /test "\$\(wc -l < "\$actual"\)" -eq 4/)
  assert.match(source, /find "\$output_root" -mindepth 1 -type d/)
  assert.equal((source.match(/name: Preserve immutable candidate [1-5] /g) || []).length, 5)
  assert.match(source, /pr-preview-signed-candidate-\$approved_pr-\$approved_head_sha/)
  assert.match(source, /Immutable signed candidate only/)
  assert.match(source, /did not write `latest`, push to CNB, or promote anything to users/)
...
test('promotion workflow owns the CNB handoff behind independent local evidence approval', async () => {
  ...
  const workflow = YAML.parse(source)
  assert.ok(workflow.on.workflow_dispatch)
  assert.equal(workflow['run-name'], 'Promote locally gated PR #${{ inputs.pull_request }} · ${{ inputs.head_sha }}')
  ...
  assert.match(source, /GIT_CONFIG_KEY_0='credential\.helper'/)
  assert.match(source, /GIT_CONFIG_VALUE_0=''/)
  assert.match(source, /GIT_CONFIG_KEY_1='credential\.helper'/)
  assert.match(source, /username=cnb/)
  assert.match(source, /password=\$CNB_PR_PREVIEW_PUSH_TOKEN/)
  assert.doesNotMatch(source, /Authorization:\s*(?:Bearer|Basic)/)
