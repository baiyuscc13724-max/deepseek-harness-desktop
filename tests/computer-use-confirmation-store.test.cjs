...
test('main process no longer installs a per-action Computer Use confirmation boundary', async () => {
  const main = await readFile(path.resolve(__dirname, '..', 'electron', 'main.cjs'), 'utf8')
  assert.doesNotMatch(main, /ComputerUseConfirmationStore|computerUseConfirmations/u)
  assert.doesNotMatch(main, /computerUse:confirm|computerUse:reject/u)
  assert.match(main, /pending: \[\]/u)
  assert.match(main, /generation: computerUseSessionGeneration/u)
  assert.match(main, /service\.globalClick\(\{ x: sourceX, y: sourceY \}\)/u)
  assert.match(main, /service\.globalType\(\{ text:/u)
  assert.doesNotMatch(main, /fingerprint = JSON\.stringify\(\{ action, x:/u)
})
