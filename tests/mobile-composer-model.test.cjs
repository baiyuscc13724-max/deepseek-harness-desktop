...
test('mobile model UI is touch-safe, bounded, and visually independent', () => {
  ...
  assert.match(compat, /\[data-harness-mobile-model-sheet\] > \[role="dialog"\][^{]*\{[^}]*height:\s*min\(76dvh, 680px\) !important;[^}]*max-height:\s*min\(76dvh, 680px\) !important;[^}]*grid-template-rows:\s*auto auto auto 44px minmax\(0, 1fr\) !important;[^}]*border-radius:\s*22px !important;/su, 'five dialog children need five tracks so the tabpanel owns the bounded scroll row')
  assert.match(modelSource, /dialog\?\.insertBefore\(handle, dialog\.firstChild\)/u, 'the drag handle is the fifth direct dialog child accounted for by the grid')
  assert.match(compat, /data-harness-mobile-model-handle="true"[^}]*grid-row:\s*1 !important;/su)
  assert.match(compat, /data-harness-mobile-model-summary[^}]*grid-row:\s*3 !important;/su)
  assert.match(compat, /\[data-harness-mobile-model-sheet\] \[role="tablist"\][^}]*grid-row:\s*4 !important;/su)
  assert.match(compat, /data-harness-mobile-model-content[^}]*grid-row:\s*5 !important;[^}]*min-height:\s*0 !important;[^}]*overflow:\s*auto !important;/su, 'the model list must start immediately below the tabs and own scrolling')
  assert.match(modelSource, /event\.target\.closest\?\.\('\[data-harness-mobile-model-content\]'\)/u, 'sheet drag handling must leave touches that begin in the list to native scrolling')
  assert.match(compat, /\[data-harness-mobile-model-sheet\] > \[role="dialog"\] > \[data-harness-mobile-model-content\][^{]*\{[^}]*overflow-x:\s*hidden !important;[^}]*overflow-y:\s*auto !important;[^}]*touch-action:\s*pan-y !important;[^}]*-webkit-overflow-scrolling:\s*touch !important;/su, 'a sheet-scoped selector must outrank the generic settings dialog overflow rule and opt into vertical touch panning')
  assert.match(compat, /\[data-harness-mobile-model-sheet\] > \[role="dialog"\] > \[data-harness-mobile-model-content\] > \[data-harness-mobile-model-choices\][^{]*\{[^}]*height:\s*auto !important;[^}]*min-height:\s*0 !important;[^}]*padding:\s*0 !important;/su, 'the generated choices list must reset the generic settings first-child 48px clamp')
  ...
  assert.match(compat, /\[data-harness-mobile-model-sheet\] \[role="tablist"\][^{]*\{[^}]*height:\s*44px !important;[^}]*min-height:\s*44px !important;[^}]*max-height:\s*44px !important;/su, 'desktop tab styles must not stretch the mobile tab row')
  assert.match(compat, /\[data-harness-mobile-model-sheet\] \[role="tablist"\] > \[data-harness-mobile-model-tab\][^{]*\{[^}]*height:\s*44px !important;[^}]*min-height:\s*44px !important;[^}]*max-height:\s*44px !important;/su, 'each model tab keeps a fixed touch-safe block size')
