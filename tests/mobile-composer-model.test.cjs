...
test('mobile model UI is touch-safe, bounded, and visually independent', () => {
  ...
  assert.match(compat, /\[data-harness-mobile-model-sheet\] > \[role="dialog"\][^{]*\{[^}]*height:\s*min\(76dvh, 680px\) !important;[^}]*max-height:\s*min\(76dvh, 680px\) !important;[^}]*grid-template-rows:\s*auto auto 44px minmax\(0, 1fr\) !important;[^}]*border-radius:\s*22px !important;/su, 'the async authoritative directory must retain a usable scroll viewport after starting from one transitional row')
