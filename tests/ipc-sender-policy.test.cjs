...
test('every privileged ipcMain handle has an explicit sender policy', async () => {
  ...
  const missing = handlerWindows(source)
    .filter(({ channel, source: window }) => !allowedSpecialCases.has(channel)
      ...
      && !window.includes('assertLocalRuntimeSender(event)'))
    .map(({ channel }) => channel)
...
test('desktop shell wrapper rejects before invoking a privileged handler', async () => {
  ...
  assert.match(source, /ipcMain\.handle\('mobileSync:getRelayConfig', desktopShellOnly\(/u)
  assert.match(source, /ipcMain\.handle\('mobileSync:setRelayConfig', desktopShellOnly\(/u)
  assert.match(source, /ipcMain\.handle\('mobileSync:clearRelayConfig', desktopShellOnly\(/u)
  assert.match(source, /ipcMain\.handle\('runtime:start', desktopShellOnly\(/u)
  assert.match(source, /ipcMain\.handle\('terminal:start', desktopShellOnly\(/u)
  assert.match(source, /ipcMain\.handle\('terminal:write', desktopShellOnly\(/u)
  assert.match(source, /ipcMain\.handle\('terminal:resize', desktopShellOnly\(/u)
  assert.match(source, /ipcMain\.handle\('terminal:stop', desktopShellOnly\(/u)
