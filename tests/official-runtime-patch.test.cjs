...
test('cache metrics separate the latest warm request from the cold-start cumulative value', async () => {
  const { patchAlpha2ConversationSources, patchTokenMeterSource } = await import('../scripts/patch-official-runtime.mjs')
  ...
  assert.match(tokenPatch.source, /key: "tokenUsageDetail"/u)
  assert.match(tokenPatch.source, /lastCacheReadReported/u)
  assert.match(tokenPatch.source, /previousPromptTokens/u)
  assert.match(tokenPatch.source, /register\(tokenUsageDetailProjectionDefinition\)/u)
  ...
  const chatFixture = readFileSync(path.resolve(__dirname, '..', 'node_modules', '@deepseek-ai', 'dsh-client-ui-chat', 'lib', 'client.js'), 'utf8')
  const patched = patchAlpha2ConversationSources(conversationFixture, chatFixture)
  assert.match(patched.chatSource, /useProjection\("tokenUsageDetail"\)/u)
  assert.match(patched.chatSource, /const runningTurnStart = useChat\(\(s\) => runningTurnStartTime\(s\.timeline\)\)/u)
  assert.match(patched.chatSource, /const followSigRef = \(0, react\.useRef\)\(null\)/u)
  assert.match(patched.chatSource, /const followSig = `\$\{openState\}:\$\{firstSeq\}:\$\{lastKey\}:\$\{order\.length\}:\$\{running \? 1 : 0\}/u)
  assert.match(patched.chatSource, /const tipMoved = followSigRef\.current !== followSig/u)
  assert.match(patched.chatSource, /if \(appendedUser \|\| appendedSteering \|\| appendedSubmission \|\| tipMoved && atBottomRef\.current\) toBottom\(el\)/u)
  assert.match(patched.chatSource, /最近一步缓存读取/u)
  assert.match(patched.chatSource, /提供方未报告/u)
  assert.match(patched.chatSource, /累计缓存读取 \{percent\}%（含首次冷启动）/u)
  assert.match(patched.chatSource, /store = new MutableChatNodeStore\(\)/u)
  assert.match(patched.chatSource, /nodes: this\.store/u)
  assert.doesNotThrow(() => new Function(patched.conversationSource))
  assert.doesNotThrow(() => new Function(patched.chatSource))
  assert.equal(patchAlpha2ConversationSources(patched.conversationSource, patched.chatSource).changed, false)
})
