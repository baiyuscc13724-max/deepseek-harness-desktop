...
test('Agent Teams plugin installs an explicitly marked artifact fixture into the Web profile idempotently', async () => {
  ...
  try {
    ...
    assert.equal(first.version, '1.0.61')
    ...
    assert.match(host, /queueHostSubagentPrompt/u)
    assert.match(host, /snapshotSessionEvents/u)
    assert.doesNotMatch(host, /session\??\.events|session\.events/u)
    assert.doesNotMatch(host, /subagents\??\.followup|subagents\.followup/u)
...
test('default desktop startup mode still installs from a source checkout while gate mode rejects it', async () => {
  ...
  try {
    ...
    assert.equal(installed.version, '1.0.61')
