...
test('cloud recovery prefetches exact bytes on Ubuntu and keeps the Windows gate read-only', () => {
  ...
  const prefetch = recovery.steps.find(step => step.name === 'Prefetch exact draft bytes and previous stable installer on Ubuntu')
  ...
  assert.match(prefetch.run, /releases\/assets\/\$asset_id/u)
  ...
  assert.match(windowsRuns, /Resolve-Path 'draft-state\/release-files'/u)
  ...
  assert.match(windowsRuns, /Downloaded files do not exactly match SHA256SUMS\.txt/u)
  assert.match(windowsRuns, /Harness-Desktop-\$version-portable-x64\.exe/u)
  assert.match(windowsRuns, /\$portableSelfTest\.ExitCode -ne 0/u)
  assert.match(windowsRuns, /Harness-Desktop-\$version-win-x64\.exe/u)
  assert.match(windowsRuns, /Previous stable installed self-test JSON failed or reported the wrong version/u)
  assert.match(windowsRuns, /Find-DllHoldingProcess[\s\S]*d3dcompiler_47\.dll/u)
  assert.match(windowsRuns, /Invoke-BoundedProcess[\s\S]*WaitForExit[\s\S]*-TimeoutSeconds 600/u)
  assert.match(windowsRuns, /\/NORESTARTAPPLICATIONS/u)
  assert.match(windowsRuns, /\/LOGCLOSEAPPLICATIONS/u)
  assert.doesNotMatch(windowsRuns, /\/FORCECLOSEAPPLICATIONS/u)
  ...
  assert.ok(publish.steps.find(step => step.name === 'Reconfirm exact draft snapshot and publish by immutable release id'))
  assert.match(publishRuns, /diff -u <\(jq -S \. draft-snapshot\.json\) <\(jq -S \. current-draft\.json\)/u)
