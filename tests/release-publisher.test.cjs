...
test('Android-only publication stays inside the unified publisher and excludes desktop/component side effects', () => {
  ...
  assert.match(publisher, /POST_TAG_CONTROLLER_PATHS[\s\S]*'package-lock\.json'/u)
  assert.match(publisher, /POST_TAG_LOCK_REPAIRS[\s\S]*'node_modules\/fast-uri'[\s\S]*from: '3\.1\.5'[\s\S]*to: '3\.1\.6'/u)
  assert.match(publisher, /POST_TAG_LOCK_REPAIRS[\s\S]*'node_modules\/qs'[\s\S]*from: '6\.15\.3'[\s\S]*to: '6\.16\.0'/u)
  assert.match(publisher, /function assertExactPostTagLockRepair[\s\S]*currentRemainder[\s\S]*baselineRemainder/u)
  assert.match(publisher, /changed\.includes\('package-lock\.json'\)[\s\S]*assertExactPostTagLockRepair/u)
  ...
  const fetchRetryStart = publisher.indexOf('function gitFetchRemoteMainWithRetry()')
  const fetchRetryEnd = publisher.indexOf('function ghCapture', fetchRetryStart)
  assert.ok(fetchRetryStart >= 0 && fetchRetryEnd > fetchRetryStart)
  const fetchRetry = publisher.slice(fetchRetryStart, fetchRetryEnd)
  assert.match(publisher, /const GIT_FETCH_RETRY_ATTEMPTS = 6/u)
  assert.match(fetchRetry, /\['fetch', 'origin', 'main', '--tags'\]/u)
  assert.match(publisher, /Recv failure:\\s\*Connection was reset/u)
  assert.match(fetchRetry, /TRANSIENT_GIT_FETCH_ERROR/u)
  assert.match(fetchRetry, /Atomics\.wait/u)
  assert.doesNotMatch(fetchRetry, /\['(?:push|tag)'/u, 'only the fixed read-only fetch may be retried')
  assert.match(publisher, /function assertExactRemoteMain\(\) \{\s*gitFetchRemoteMainWithRetry\(\)/u)
  const remoteRetryStart = publisher.indexOf('const REMOTE_READ_RETRY_ATTEMPTS = 4')
  const remoteRetryEnd = publisher.indexOf('async function protectedMetadataHashes', remoteRetryStart)
  assert.ok(remoteRetryStart >= 0 && remoteRetryEnd > remoteRetryStart)
  const remoteRetry = publisher.slice(remoteRetryStart, remoteRetryEnd)
  assert.match(remoteRetry, /UND_ERR_CONNECT_TIMEOUT/u)
  assert.match(remoteRetry, /withRemoteReadRetry/u)
  assert.match(remoteRetry, /fetchRemoteRead/u)
  assert.match(remoteRetry, /response\.arrayBuffer\(\)/u)
  assert.match(remoteRetry, /for await \(const chunk of response\.body\)/u)
  assert.doesNotMatch(remoteRetry, /method:\s*['"](?:POST|PUT|PATCH|DELETE)['"]/u, 'only remote reads may be retried')
  assert.equal((publisher.match(/await fetch\(/gu) || []).length, 2, 'every direct fetch is enclosed by a read-only retry')
