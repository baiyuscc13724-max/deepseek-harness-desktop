...
test('cloud recovery binds artifacts to the tag and safely resumes any verified subset', () => {
  ...
  assert.match(windowsRuns, /function Invoke-BoundedDownload[\s\S]*CancellationTokenSource[\s\S]*CopyToAsync\(\$target, 1048576, \$cts\.Token\)[\s\S]*Authenticated release asset download timed out/u)
