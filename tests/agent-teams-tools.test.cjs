...
test('failed-member recovery tool auto-admits exact safe Goal rounds while trusted Host actions keep OCC', async () => {
  const host = await readFile(hostFile, 'utf8')
  const tool = sliceBetween(host, 'name: "team_member_recover"', 'name: "team_member_reconcile"')
  const reconcileTool = sliceBetween(host, 'name: "team_member_reconcile"', 'name: "team_status"')
  assert.doesNotMatch(tool, /requireDirectHumanRoot\(ctx, execution\)/u)
  assert.match(tool, /hasDirectHumanRootAuthority\(ctx, execution\)/u)
  assert.match(tool, /exactGoalRoundRootAuthority\(ctx, execution\)/u)
  assert.match(tool, /automaticContinuation: !directHuman/u)
  assert.match(tool, /request_id/u)
  assert.match(tool, /expected_revision/u)
  assert.match(tool, /recoverFailedMember/u)
  assert.match(reconcileTool, /requireDirectHumanRoot\(ctx, execution\)/u)
  assert.match(reconcileTool, /reconcileMemberRecovery/u)
  assert.match(reconcileTool, /resolution/u)
  ...
  assert.match(host, /assertAutomaticMemberRecoveryAllowed/u)
  assert.match(host, /team changed before member recovery; refresh current state and retry/u)
  assert.match(host, /the safe automatic retry was already used for this failed member; replace it without asking the user/u)
  assert.match(host, /safe active autopilot grant automatically authorizes/u)
