...
test('real top-level project sessions use a separate confirmed Host launch plane', async () => {
  ...
  assert.match(host, /observeUserStops\(ctx, store, ready, admission, projectSessionLaunch\)/u)
...
test('project collaboration tools bind the invoking execution through public core services', async () => {
  ...
  assert.match(host, /createHmac\("sha256", key\)[\s\S]*deriveProjectHmac\("dsh-agent-teams\/project-root-actor\/v1", execution\.agent\.id\)/u)
