...
test('策略文件路径安全：拒绝 URL、控制字符、越界路径', () => {
  ...
  assert.equal(assertSafePolicyPath('D:\\data\\policy.json', { rootDir: 'D:\\data' }), path.resolve('D:\\data\\policy.json'))
  ...
  assert.throws(() => assertSafePolicyPath('D:\\a\u0000b.json'), error => error.code === 'invalid-path')
  ...
  assert.throws(() => assertSafePolicyPath('D:\\outside.json', { rootDir: root }), error => error.code === 'path-escape') // 根目录之外 → 逃逸
  assert.throws(() => assertSafePolicyPath('..\\outside.json', { rootDir: root }), error => error.code === 'path-escape')
  assert.throws(() => assertSafePolicyPath(`D:\\x\\${'y'.repeat(MAX_POLICY_PATH_LENGTH)}.json`), error => error.code === 'invalid-path')
})