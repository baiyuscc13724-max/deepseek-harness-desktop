...
test('selected board task becomes the visual focus with a truthful live workflow', async () => {
  ...
  assert.match(focus, /var connection = props\.connection/u)
  ...
  assert.match(workflow, /className: "dat-task-block-branch", "data-active": workflow\.blocked/u)
  ...
  for (const copy of ['实时工作流', '阻塞不是必经阶段', 'Live workflow', 'Blocking is not a required stage']) assert.ok(source.includes(copy), `missing focused-detail copy: ${copy}`)
})
