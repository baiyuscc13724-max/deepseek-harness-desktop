...
test('custom wallpaper workbench protects conversation chrome, content, composer and inline code', () => {
  ...
  assert.match(guest, /html\[data-hd-theme="custom"\]\[data-hd-skin-tone\] \[data-composer-card="true"\],/u)
  assert.match(guest, /background:linear-gradient\(var\(--hd-theme-readable-surface\),var\(--hd-theme-readable-surface\)\),var\(--hd-theme-input\) !important/u)
