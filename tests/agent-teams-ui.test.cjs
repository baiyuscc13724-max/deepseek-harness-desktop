...
test('settings start from the same safe defaults as the host', async () => {
  const source = await clientSource()
  assert.match(source, /useState\(\{ enabled: false, maxMembers: 4, maxActiveTurns: 4, autopilotEnabled: false, autopilotMaxAdditionalRounds: 200 \}\)/u)
  ...
  assert.match(source, /只调整成员上限可直接保存/u)
  assert.match(source, /可信偏好可用于之后新建的安全团队/u)
  ...
  assert.match(source, /var authorizationRequired = !!values\.autopilotEnabled \|\| !!values\.autopilotEnabled !== committedAutopilotEnabled\.current \|\| budget !== committedAutopilotBudget\.current/u)
  assert.match(source, /settingsAutopilotScopeRequired: "开启自动接力需要当前负责人、活动 Goal 与安全团队的精确 Host 授权/u)
  assert.match(source, /settingsAutopilotScopeRequired: "Enabling automatic continuation requires exact Host authorization for the current lead, active Goal, and safe team/u)
  assert.match(source, /if \(authorizationRequired && values\.autopilotEnabled && !hostAuthorization\)/u)
