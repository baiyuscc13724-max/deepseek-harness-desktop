...
test('CNB preview branch mirrors immutable assets before atomically promoting signed feeds', async () => {
  ...
  assert.match(stages[0].script, /localGateEvidence\.kind == "harness-pr-preview-local-gate-evidence"/)
  assert.match(stages[0].script, /localGateEvidence\.candidate\.componentSha256 == \$componentSha/)
  assert.match(stages[0].script, /evidenceSha256 == \$evidenceSha/)
...
test('cloud feed verifier fails closed until a production preview public key is provisioned', async t => {
  ...
  const production = JSON.parse(await source('pr-preview-update-sources.json'))
  await writeFile(config, JSON.stringify({ ...production, enabled: true, trustedKeys: {} }))
  await writeFile(index, '{}')
  await writeFile(manifest, '{}')
  const { verifyPrPreviewFeedFiles } = await import('../scripts/pr-preview-verify-feed.mjs')
  await assert.rejects(() => verifyPrPreviewFeedFiles({ configFile: config, indexFile: index, manifestFile: manifest }), /必须配置独立公钥/)
})
