...
test('desktop themes lock the outer viewport while preserving mobile page scrolling', async () => {
  ...
  assert.match(source, /applyUiMode\(false\)\s*if \(refreshTheme\) applyTheme\(\)\s*stabilizeWorkbenchViewport\(\)/u)
  assert.match(source, /const sourceChanged = video\.dataset\.hdWallpaperSource !== source/u)
  assert.match(source, /releaseWallpaperVideo\(video\)\s*video\.dataset\.hdWallpaperSource = source\s*video\.src = source\s*video\.load\(\)/u)
  assert.match(source, /if \(!video\.paused\) video\.pause\(\)/u)
  assert.match(source, /else if \(video\.paused \|\| sourceChanged\) video\.play\(\)\.catch/u)
  assert.match(source, /releaseWallpaperVideo\(video\)\s*video\.remove\(\)/u)
