...
test('v6 stores one canonical snapshot and only bounded lossless deltas while get returns one immutable projection', () => {
  ...
  const firstRuntimeHeartbeat = readFileSync(store.runtimeFile, 'utf8')
  store.touchDevice('0123456789abcdef', new Date('2026-09-01T00:01:00.500Z'))
  assert.equal(store.get().devices[0].lastSeenAt, '2026-09-01T00:01:00.500Z')
  assert.equal(readFileSync(store.runtimeFile, 'utf8'), firstRuntimeHeartbeat, 'sub-second heartbeat bursts reuse the most recent durable runtime record')
  store.touchDevice('0123456789abcdef', new Date('2026-09-01T00:01:01.000Z'))
  assert.equal(JSON.parse(readFileSync(store.runtimeFile, 'utf8')).heartbeats[0].lastSeenAt, '2026-09-01T00:01:01.000Z')
  store.setPreferredPort(4567)
