...
test('system and edge back use the fixed runtime protocol without double dispatch', () => {
  ...
  assert.match(activity, /WORKBENCH_PRIORITY = OnBackInvokedDispatcher\.PRIORITY_OVERLAY/)
  assert.match(activity, /registerOnBackInvokedCallback\(WORKBENCH_PRIORITY, callback\)/)
