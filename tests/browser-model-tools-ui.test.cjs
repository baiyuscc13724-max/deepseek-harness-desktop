...
test('browser model tools remain user-authorized, background-capable, structured and secret-blind', async () => {
  ...
  assert.match(main, /surface: ready \? visible \? 'visible' : 'background' : contents \? 'unavailable' : 'not-started'/u)
  ...
  assert.match(html, /Computer Use 与浏览器控制/u)
  assert.match(html, /这一次授权同时开启右栏浏览器控制与内置 Computer Use/u)
  assert.match(html, /右栏浏览器仍保留密码、账号、验证码、支付和银行信息等硬限制/u)
  assert.match(html, /设置 → 插件 → Computer Use/u)
  assert.match(html, /浏览器控制复用 Computer Use 授权/u)
