...
test('registered project tools isolate 16 canonical roots with identical model ids and independent slow/fast SQLite lanes', async () => {
  ...
  try {
    ...
    sessionRuntimeResolver = mod.createProjectTaskSessionRuntimeResolver(ctx, registry)
    ...
    assert.throws(() => sessionRuntimeResolver(), error => error.code === 'PROJECT_TASK_WEB_FORBIDDEN')
    assert.throws(() => sessionRuntimeResolver('unknown-root'), error => error.code === 'PROJECT_TASK_WEB_FORBIDDEN')
    const scopedRuntime = sessionRuntimeResolver(roots[0].id)
    assert.equal(scopedRuntime.projectEntry.status, undefined, 'session-scoped lanes must not consult device-global project status')
    ...
    const otherScopedState = await sessionRuntimeResolver(roots[1].id).state()
    assert.equal(otherScopedState.projectCollaboration.sections.tasks.some(task => task.taskRef === taskRefs[1]), true)
    assert.equal(otherScopedState.projectCollaboration.sections.tasks.some(task => task.taskRef === taskRefs[0]), false)
    assert.equal(otherScopedState.capability.legacyTeamTasks.detected, false, 'a scoped panel must not expose another project\'s Agent Team presence')
