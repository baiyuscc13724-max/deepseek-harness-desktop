...
test('Computer Use is built in with pushed session/permanent unlimited authorization', async () => {
  ...
  try {
    ...
    assert.match(renderer, /computer-use-refresh/u)
    assert.match(renderer, /computer-use-status/u)
    assert.match(renderer, /computer-use-toggle/u)
    assert.match(renderer, /computer-use-revoke-permanent/u)
