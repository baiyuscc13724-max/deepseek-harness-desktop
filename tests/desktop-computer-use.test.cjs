...
test('Computer Use is built in with pushed session/permanent unlimited authorization and permanent auto-restore', async () => {
  ...
  try {
    ...
    const loadedPlugin = await import(`${pathToFileURL(path.join(bundledRoot, 'lib', 'index.js')).href}?alpha4-settings=${Date.now()}`)
    ...
    assert.equal(manifest.peerDependencies['@deepseek-ai/dsh-settings'], '^0.1.2-alpha.4')
    assert.equal(manifest.peerDependencies['@deepseek-ai/dsh-tools'], '^0.1.2-alpha.4')
