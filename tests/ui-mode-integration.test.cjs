...
test('mode material effects are scoped, reversible and performance aware', () => {
  ...
  assert.match(guest, /\[data-slot="sidebar"\] > \*:has\(\[role="dialog"\]\[aria-modal="true"\]\) \{ backdrop-filter:none!important; \}/)
  ...
  assert.match(guest, /transform:translateY\(-6px\) scale\(1\.015\)/)
