...
test('native P2P renderer gates negotiation and bounds every untrusted path', () => {
  ...
  assert.match(androidSource, /signalingV2 = true;\s*(?:\/\/[^\n]*\s*)*openSocksServer\(activeGeneration\);\s*timer\.schedule/u)
...
test('Android release shrinker preserves every WebRTC JNI binding symbol', () => {
  const root = path.join(__dirname, '..')
  const rules = readFileSync(path.join(root, 'mobile', 'android', 'app', 'proguard-rules.pro'), 'utf8')
  const gradle = readFileSync(path.join(root, 'mobile', 'android', 'app', 'build.gradle.kts'), 'utf8')
  assert.match(gradle, /release \{[\s\S]*isMinifyEnabled = true/u)
  assert.match(gradle, /implementation\("io\.github\.webrtc-sdk:android:[^"]+"\)/u)
  assert.match(gradle, /tasks\.register\("verifyReleaseWebRtcJniSymbols"\)[\s\S]*PeerConnectionFactory\\\$InitializationOptions[\s\S]*NativeLibraryLoader[\s\S]*org\.jni_zero\.JniInit/u)
  assert.match(gradle, /name == "assembleRelease"\) dependsOn\(verifyReleaseWebRtcJniSymbols\)/u)
  assert.match(rules, /-keep class org\.webrtc\.\*\* \{ \*; \}/u)
  assert.match(rules, /-keep class org\.jni_zero\.\*\* \{ \*; \}/u)
  assert.match(rules, /-keepattributes \*Annotation\*/u)
})
